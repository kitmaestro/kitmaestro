import { Component, signal, inject, ChangeDetectionStrategy, OnInit, OnDestroy, ViewEncapsulation, computed } from '@angular/core';
import { FormBuilder, ReactiveFormsModule, Validators, AbstractControl } from '@angular/forms';
import { CommonModule } from '@angular/common';
import { Subject, firstValueFrom, takeUntil, tap, catchError, EMPTY, finalize, switchMap, filter, map, distinctUntilChanged } from 'rxjs';
...
import { ClassSectionService } from '../services/class-section.service'; // Service for sections
import { SubjectConceptListService } from '../services/subject-concept-list.service'; // Service for concepts
...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="sports-practice-card"> <mat-card-header>
    <mat-card-title>Generador de Prácticas Deportivas</mat-card-title>
    <mat-card-subtitle>Obtén planes de entrenamiento detallados para tus clases</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="sportsPracticeForm" (ngSubmit)="onSubmit()" class="sports-practice-form">
        <div class="form-row">
        ...
        <div class="form-row">
           <mat-form-field appearance="outline" class="form-field">
            <mat-label>Disciplina Deportiva / Concepto</mat-label>
            <mat-select formControlName="disciplineConcept" required>
               @if(isLoadingConcepts()){
                <mat-option disabled><mat-spinner diameter="20" class="inline-spinner"></mat-spinner> Cargando...</mat-option>
                 ...
                 @if(!availableConcepts().length && subjectCtrl?.valid && !isLoadingConcepts()){
                   <mat-option disabled>No hay conceptos/disciplinas para esta asignatura/grado.</mat-option>
                   <mat-option [value]="'Otra'">{{ 'Otra' }}</mat-option> } @else if (availableConcepts().length > 1) {
                    <mat-option [value]="'Otra'">{{ 'Otra' }}</mat-option>
                 }
               }
               @if(!subjectCtrl?.valid){
                   <mat-option disabled>Selecciona sección y asignatura primero.</mat-option>
               }
            </mat-select>
             @if (disciplineConceptCtrl?.invalid && disciplineConceptCtrl?.touched) {
              <mat-error>Selecciona o escribe una disciplina.</mat-error>
            }
          </mat-form-field>

           @if (disciplineConceptCtrl?.value === 'Otra') {
            <mat-form-field appearance="outline" class="form-field">
              <mat-label>Escribe la disciplina personalizada</mat-label>
              <input matInput formControlName="customDiscipline" required placeholder="Ej: Ultimate Frisbee, Yoga Infantil">
               @if (customDisciplineCtrl?.invalid && customDisciplineCtrl?.touched) {
                <mat-error>Ingresa la disciplina personalizada.</mat-error>
              }
            </mat-form-field>
          }
        </div>
        ...
        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="sportsPracticeForm.invalid || isGenerating()">
            @if (isGenerating()) {
              <mat-spinner diameter="20" color="accent" class="inline-spinner"></mat-spinner>
              Generando...
            } @else {
				<ng-container>
              <mat-icon>fitness_center</mat-icon> Generar Plan de Práctica
			</ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="sports-practice-result">
        ...
        <div class="result-actions">
          ...
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedPlan() || generatedPlan().startsWith('Ocurrió un error')">
  ...
  styles: [`
    ...
    .sports-practice-card { /* No max-width */
      margin: 0 auto;
...
export class SportsPracticeGeneratorComponent implements OnInit, OnDestroy {
  ...
  #listenForSectionChanges(): void {
   ...
   #listenForSubjectChanges(): void {
     this.subjectCtrl?.valueChanges.pipe(
       takeUntil(this.#destroy$),
       ...
       switchMap(subject => { // Use switchMap to handle async concept loading
         ...
         return this.#conceptService.findAll(filterParams).pipe(
           ...
           map(conceptLists => {
             ...
             const concepts = conceptLists?.flatMap(list => list.concepts || []) || [];
             return [...new Set(concepts)]; // Unique concepts
           }),
           catchError(error => this.#handleError(error, 'Error al cargar disciplinas/conceptos.')),
  ...
  async onSubmit(): Promise<void> {
    ...
    if (!discipline) {
        this.#snackBar.open('Por favor, selecciona o introduce una disciplina válida.', 'Cerrar', { duration: 3000 });
    ...
    try {
      ...
      this.generatedPlan.set(result?.response || 'No se pudo generar el plan de práctica.');
      this.showResult.set(true);
    } catch (error) {
       this.generatedPlan.set('Ocurrió un error al generar el plan. Por favor, inténtalo de nuevo.');
  ...
  downloadDocx(): void {
    ...
    const disciplineName = (discipline || 'Disciplina').substring(0,20).replace(/[^a-z0-9]/gi, '_');
      ...
      sections: [
        {
          properties: {},
          children: [
              new Paragraph({ children: [new TextRun({ text: `Plan de Práctica Deportiva`, bold: true, size: 28 })], spacing: { after: 300 } }),
              new Paragraph({ children: [new TextRun({ text: `Disciplina: ${discipline}`, bold: true, size: 26 })], spacing: { after: 200 } }),
              new Paragraph({ children: [new TextRun({ text: `Sección: ${this.getSectionDisplay(section!)}`, size: 24 })], spacing: { after: 100 } }),
              new Paragraph({ children: [new TextRun({ text: `Asignatura: ${formValue.subject}`, size: 24 })], spacing: { after: 400 } }),
  ...
  get sectionCtrl(): AbstractControl | null { return this.sportsPracticeForm.get('section'); }
  get subjectCtrl(): AbstractControl | null { return this.sportsPracticeForm.get('subject'); }
  get disciplineConceptCtrl(): AbstractControl | null { return this.sportsPracticeForm.get('disciplineConcept'); }
  get customDisciplineCtrl(): AbstractControl | null { return this.sportsPracticeForm.get('customDiscipline'); }
}
