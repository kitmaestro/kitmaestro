...
import { AiService } from '../services/ai.service'; // Adjust path if necessary
...
@Component({
  ...
  <mat-card-header>
    ...
    <mat-card-subtitle>Crea reflexiones personalizadas para tus estudiantes</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!mostrarReflexion()) {
      <form (ngSubmit)="generarReflexion()" class="reflection-form">
        <div class="form-row">
          <mat-form-field appearance="outline" class="form-field">
            <mat-label>Nivel Educativo</mat-label>
            <mat-select [(ngModel)]="nivelSeleccionado" name="nivel" required #nivelInput="ngModel">
          ...
          <mat-form-field appearance="outline" class="form-field">
            ...
            <mat-select [(ngModel)]="gradoSeleccionado" name="grado" required #gradoInput="ngModel" [disabled]="!nivelSeleccionado()">
              @for (grado of gradosDisponibles(); track grado) {
                <mat-option [value]="grado">{{ grado }}</mat-option>
              }
            </mat-select>
             @if (!nivelSeleccionado()) {
                <mat-hint>Selecciona un nivel primero</mat-hint>
             }
        ...
        <div class="form-row">
          <mat-form-field appearance="outline" class="form-field theme-select">
            <mat-label>Tema</mat-label>
            <mat-select [(ngModel)]="temaSeleccionado" name="tema" required #temaInput="ngModel">
               @for (tema of temasPredefinidos(); track tema) {
          ...
          @if (temaSeleccionado() === 'Otro') {
            <mat-form-field appearance="outline" class="form-field custom-theme">
              <mat-label>Escribe tu tema personalizado</mat-label>
              <input matInput [(ngModel)]="temaPersonalizado" name="temaPersonalizado" required #customThemeInput="ngModel">
               @if (customThemeInput.invalid && customThemeInput.touched) {
                <mat-error>Ingresa un tema personalizado.</mat-error>
              }
            </mat-form-field>
          }
        </div>
        ...
        <div class="form-row">
          <mat-form-field appearance="outline" class="form-field theme-select">
            ...
            <mat-select [(ngModel)]="formatoSeleccionado" name="formato" required #formatInput="ngModel">
               @for (f of formato; track f) {
                <mat-option [value]="f">{{ f }}</mat-option>
              }
            </mat-select>
             @if (formatInput.invalid && formatInput.touched) {
              <mat-error>Selecciona un formato.</mat-error>
            }
          </mat-form-field>
          <mat-form-field appearance="outline" class="form-field theme-select">
            <mat-label>Tono</mat-label>
            <mat-select [(ngModel)]="tonoSeleccionado" name="tema" required #tonoInput="ngModel">
        ...
        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="!esFormularioValido() || estaGenerando()">
            @if (estaGenerando()) {
    ...
    @if (mostrarReflexion()) {
      <div class="reflection-display">
        ...
        <div class="carta" [innerHTML]="reflexionGenerada().replaceAll('\n', '<br>')">
          </div>
        ...
        <div class="reflection-actions">
          ...
          <button mat-raised-button color="primary" (click)="descargarDocx()" [disabled]="!reflexionGenerada() || reflexionGenerada().startsWith('Ocurrió un error')">
...
export class ReflectionGeneratorComponent {
  ...
  async generarReflexion(): Promise<void> {
    ...
    const prompt = `Eres un asistente educativo especializado en crear contenido pedagógico.
	  ...
	  this.#aiService.geminiAi(prompt).subscribe({
		  next: res => {
			  this.reflexionGenerada.set(res.response || 'No se pudo generar la reflexión.'); // Set reflection or default message
  ...
  descargarDocx(): void {
    ...
    const tema = this.temaFinal().substring(0, 20).replace(/[^a-z0-9]/gi, '_'); // Limit length and sanitize
