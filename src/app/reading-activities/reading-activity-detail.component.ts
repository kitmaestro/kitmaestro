...
@Component({
  ...
  template: `@if (activity) {
    ...
    <mat-dialog-content>
        <div id="reading-activity" style="padding: 0.5in">
            <div style="text-align: center;">
                <h2 style="margin-bottom: 2px; line-height: 1;">{{activity.section.school.name}}</h2>
                <h4 style="margin-bottom: 2px; line-height: 1;">A&ntilde;o Escolar {{schoolYear}}</h4>
                <h3 style="margin-bottom: 2px; line-height: 1;">{{activity.user.title}}. {{activity.user.firstname}} {{activity.user.lastname}}</h3>
            ...
            <h3 style="font-size: 14pt; font-weight: bold;">{{activity.title}}</h3>
            <p style="font-size: 12pt; margin-top: 12px; margin-bottom: 12px;">{{activity.text}}</p>
    ...
    <mat-dialog-actions>
        ...
        <button [disabled]="printing" mat-button (click)="download()">Descargar</button>
    </mat-dialog-actions>
}`,
  styles: `.blank {
