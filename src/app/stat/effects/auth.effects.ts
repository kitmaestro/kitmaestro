...
export class AuthEffects {
  ...
  load$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(() => this.authService.profile().pipe(
  ...
  login$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(action => this.authService.login(action.email, action.password).pipe(
      map((response) => AuthActions.loginSuccess(response as any)),
  ...
  update$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(action => this.authService.update(action.user).pipe(
      map((result) => {
        console.log(result)
        this.sb.open('Tus ajustes han sido actualizados.', 'Ok', { duration: 2500 });
        AuthActions.load();
