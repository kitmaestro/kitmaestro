...
export class AnnualPlanGeneratorComponent implements OnInit {
    ...
    onSectionOrSubjectChange(): void {
        ...
        const selectedSection = this.classSections.find(s => s._id === sectionId);
        if (selectedSection) {
			this.competenceService.findAll({ grade: selectedSection.year, level: selectedSection.level, subject }).subscribe({
				next: (value) => {
					if (value.length) {
            ...
            this.contentBlockService.findAll({
                ...
                this.contentBlocks = blocks.sort((a, b) => a.order - b.order);
            });
        }
    }
    ...
    async generateYearlyPlan(): Promise<void> {
        ...
        if (contentUnits.length === 0) {
            this.sb.open('No se encontraron contenidos para la asignatura y grado seleccionados.', 'Ok', { duration: 3000 });
        ...
        for (const unitContents of contentUnits) {
            try {
                ...
                this.sb.open(`Error al generar la unidad ${this.plansGenerated + 1}. Intentando con la siguiente.`, 'Ok', { duration: 4000 });
            }
        }

        this.generating = false;
        this.sb.open('¡Planificación anual generada y guardada con éxito!', 'Ok', { duration: 5000 });
    ...
    private divideContentIntoUnits(): ContentBlock[][] {
        ...
        for (let i = 0; i < units; i++) {
            let contentsInThisUnit = baseContentsPerUnit + (remainder > 0 ? 1 : 0);
            result.push(this.contentBlocks.slice(currentIndex, currentIndex + contentsInThisUnit));
    ...
    private async generateAndSaveUnitPlan(unitContents: ContentBlock[]): Promise<void> {
        ...
        const learningSituationText = this.getLearningSituationPrompt(unitContents, classSection, environment!, situationType!, reality!, mainTheme!);
        const situationResponse = await this.aiService.geminiAi(learningSituationText).toPromise();

        const situationJson = this.extractJson(situationResponse?.response || '{}');
        const { title, content: learningSituationContent, strategies } = situationJson;

        // 2. Generar Secuencia de Actividades
        const activitiesText = this.getActivitiesPrompt(unitContents, classSection, mainTheme!, resources as string[], learningSituationContent, subject);
        const activitiesResponse = await this.aiService.geminiAi(activitiesText).toPromise();
        const activitiesJson = this.extractJson(activitiesResponse?.response || '{}');
        ...
        const plan: Partial<UnitPlan> = {
			...
			mainThemes: this.mainThemes.filter(t => t.category === mainTheme).map(t => t._id),
            subjects: [subject],
    ...
    private getLearningSituationPrompt(contents: ContentBlock[], classSection: ClassSection, environment: string, situationType: string, reality: string, mainTheme: string): string {
        const contentStr = this.formatContentsForPrompt(contents);
        return generateLearningSituationPrompt
            .replace('nivel_y_grado', `${classSection.year.toLowerCase()} de ${classSection.level.toLowerCase()}`)
    ...
    private getActivitiesPrompt(contents: ContentBlock[], classSection: ClassSection, mainTheme: string, resources: string[], learningSituation: string, subject: string): string {
        const contentStr = this.formatContentsForPrompt(contents);
    ...
    private extractJson(text: string): any {
        try {
            ...
            console.error("Error al parsear JSON de la respuesta de la IA:", error, "Texto original:", text);
