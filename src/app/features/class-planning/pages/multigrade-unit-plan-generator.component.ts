...
@Component({
                        ...
                        <form [formGroup]="learningSituationForm" style="padding-top: 16px">
                            ...
                            <div class="cols-2">
                                <mat-form-field appearance="outline">
                                    ...
                                    <mat-select formControlName="mainThemeCategory" required>
                                ...
                                <mat-form-field appearance="outline">
                                    ...
                                    <mat-select formControlName="classSections" required multiple (selectionChange)="onSectionSelect()">
                                        @for (section of allClassSections; track section._id) {
                                ...
                                <mat-form-field appearance="outline">
                                    ...
                                    <mat-select formControlName="subject" required (selectionChange)="onSubjectSelect()">
                                        @for (subject of commonSubjects; track subject.id) {
                            ...
                            <!-- Dynamic Content Blocks per Grade -->
                             @for (section of selectedClassSections; track section._id) {
                                <div>
                                    <h4 class="bold">Contenidos para {{ section.name }}</h4>
                                    <mat-form-field appearance="outline">
                                        <mat-label>Seleccionar Contenidos</mat-label>
                                        <mat-select [formControlName]="'content_' + section._id" multiple required>
                                            @for (content of contentBlocksBySection.get(section._id); track content._id) {
                            ...
                            @if (learningSituation.value) {
                                <div style="margin-top: 16px; margin-bottom: 16px">
                                    <h3 style="font-weight: bold">
                                        Situaci&oacute;n de Aprendizaje: {{ learningSituationTitle.value }}
                                    </h3>
                                    <mat-form-field appearance="outline">
                    ...
                    <mat-step [stepControl]="unitPlanForm">
                         <form [formGroup]="unitPlanForm">
                            ...
                            <div style="padding-top: 16px">
                                        ...
                                        <mat-select formControlName="duration" required>
                                            @for (n of [].constructor(6); track $index) {
                                                <mat-option [value]="$index + 1"
                                                    >{{ $index + 1 }} Semana{{ $index > 0 ? "s" : "" }}</mat-option
                                                >
                                            }
                                        </mat-select>
                                    </mat-form-field>
                                ...
                                <div>
                                    <mat-form-field appearance="outline">
                                        <mat-label>Metodolog&iacute;a Principal</mat-label>
                                        <mat-select formControlName="teaching_method" required>
                                            @for (method of teachingMethods; track $index) {
                                ...
                                <div style="margin-bottom: 16px">
                                    ...
                                    <mat-chip-listbox formControlName="resources" multiple (selectionChange)="onResourceChange($event)">
                            ...
                            <div style="text-align: end">
                                ...
                                <button style="margin-left: 8px" mat-raised-button (click)="generateActivities()" matStepperNext>
                                    Siguiente
                                </button>
                            </div>
                        </form>
                    ...
                    <mat-step>
                        ...
                        <div style="padding-top: 16px">
                            @if (activitiesByGrade.length) {
                                @for(gradeActivities of activitiesByGrade; track gradeActivities.grade_level) {
                                    <div class="grade-activities-container">
                                        <h3>Actividades para {{ gradeActivities.grade_level }}</h3>
                                        <div class="flex-on-md">
                                            <div style="flex: 1 1 auto; width: 100%">
                                                <h4>Actividades de Ense&ntilde;anza</h4>
                                                <ul>
                                                    @for(activity of gradeActivities.teacher_activities; track activity) {
                                                        <li>{{ activity }}</li>
                                                    }
                                                </ul>
                                            </div>
                                            <div style="flex: 1 1 auto; width: 100%">
                                                ...
                                                <ul>
                                                    @for(activity of gradeActivities.student_activities; track activity) {
                                                        <li>{{ activity }}</li>
                                                    }
                                                </ul>
                                            </div>
                                            <div style="flex: 1 1 auto; width: 100%">
                                                <h4>Actividades de Evaluaci&oacute;n</h4>
                                                <ul>
                                                    @for(activity of gradeActivities.evaluation_activities; track activity) {
                                                        <li>{{ activity }}</li>
                                                    }
                                                </ul>
                                            </div>
                        ...
                        <div style="text-align: end">
                            ...
                            @if (activitiesByGrade.length) {
                                <button style="margin-left: 8px" color="primary" mat-raised-button (click)="fillFinalForm()" type="button">
                                    Guardar
                                </button>
                            }
                        </div>
    ...
    styles: [`
        ...
        @media screen and (min-width: 960px) {
            ...
            .flex-on-md { display: flex; gap: 16px; margin-top: 16px; }
        }
...
export class MultigradeUnitPlanGeneratorComponent implements OnInit {
    ...
    unitPlanForm = this.fb.group({
        ...
        teaching_method: ['Aprendizaje Basado en Competencias', Validators.required],
        ...
        this.classSectionService.findSections().subscribe({
            next: (value) => {
                if (value.length) {
                    ...
                    this.sb.open('Para usar esta herramienta, necesitas crear al menos una sección.', 'Ok');
    ...
    onSectionSelect(): void {
        const selectedIds = this.learningSituationForm.get('classSections')?.value || [];
        this.selectedClassSections = this.allClassSections.filter(s => selectedIds.includes(s._id));

        // Remove old dynamic controls
        Object.keys(this.learningSituationForm.controls).forEach((key: string) => {
        ...
        this.selectedClassSections.forEach(section => {
            this.learningSituationForm.addControl('content_' + section._id as any, this.fb.control([], Validators.required));
        });
    ...
    calculateCommonSubjects(): void {
        ...
        const subjectSets = this.selectedClassSections.map(section => new Set(section.subjects));
    ...
    loadContentBlocks(): void {
        ...
        const observables = this.selectedClassSections.map(section => {
            return this.contentBlockService.findAll({ level: section.level, year: section.year, subject })
        });
        ...
        forkJoin(observables).subscribe(results => {
            ...
            results.forEach((blocks, index) => {
                ...
                this.contentBlocksBySection.set(sectionId, blocks.sort((a, b) => a.order - b.order));
            });
        });

        const competence$ = this.selectedClassSections.map(section => this.competenceService.findAll({ level: section.level, grade: section.year, subject }));
        ...
        const mainThemes$ = this.selectedClassSections.map(section => this.mainThemeService.findAll({ level: section.level, year: section.year, subject }));
    ...
    onResourceChange(event: any) {
        localStorage.setItem('available-resources', JSON.stringify(event.value));
    }
    ...
    getSelectedContentsText(): string {
        ...
        return this.selectedClassSections.map(section => {
            const contentIds = this.learningSituationForm.get('content_' + section._id)?.value || [];
            const contents = this.contentBlocksBySection.get(section._id)
                ?.filter(cb => contentIds.includes(cb._id)) || [];
            
            if (contents.length === 0) return `Para ${section.name}:\nNo se han seleccionado contenidos.`;

            const concepts = contents.flatMap(c => c.concepts).join('\n- ');
            const procedures = contents.flatMap(c => c.procedures).join('\n- ');
            const attitudes = contents.flatMap(c => c.attitudes).join('\n- ');
    ...
    generateLearningSituation(): void {
        ...
        const { environment, situationType, reality, mainThemeCategory } = this.learningSituationForm.value;
        const contentsText = this.getSelectedContentsText();
        const gradesText = this.selectedClassSections.map(s => s.name).join(', ');
        ...
        this.aiService.geminiAi(prompt).subscribe({
            next: (res) => {
                ...
                try {
                    const extract = res.response.slice(res.response.indexOf('{'), res.response.lastIndexOf('}') + 1);
                    ...
                    result.strategies.forEach((s: string) => this.strategies.push(this.fb.control(s)));
                } catch (error) {
                    this.sb.open('Error al procesar la respuesta de la IA. Inténtalo de nuevo.', 'Ok');
                }
            },
            error: () => {
                this.generating = false;
                this.sb.open('Ocurrió un error al generar la situación de aprendizaje.', 'Ok');
            }
        });
    }
    ...
    generateActivities(): void {
        this.generating = true;
        const { duration, resources, teaching_method } = this.unitPlanForm.value;
        const gradesText = this.selectedClassSections.map(s => s.name).join(', ');
        const contentsText = this.getSelectedContentsText();
        const subjectName = this.pretifyPipe.transform(this.learningSituationForm.get('subject')?.value || '');
        ...
        const prompt = generateMultigradeActivitySequencePrompt
            ...
            .replace('theme_axis', this.learningSituationForm.get('mainThemeCategory')?.value!)
        ...
        this.aiService.geminiAi(prompt).subscribe({
            next: (res) => {
                ...
                try {
                    ...
                    this.sb.open('Error al procesar las actividades generadas. Inténtalo de nuevo.', 'Ok');
                }
            },
            error: () => {
                this.generating = false;
                this.sb.open('Ocurrió un error al generar las actividades.', 'Ok');
            }
        });
    }
    ...
    fillFinalForm(): void {
         const flatActivities = (type: 'teacher' | 'student' | 'evaluation') => {
            return this.activitiesByGrade.map(g => ({
                ...
                activities: g[`${type}_activities`].map(act => `(${g.grade_level}) ${act}`),
            }));
        };
        ...
        const plan: any = {
            ...
            mainThemeCategory: this.learningSituationForm.get('mainThemeCategory')?.value,
            mainThemes,
            subjects: [this.learningSituationForm.get('subject')?.value],
            strategies: this.strategies.value,
            contents: this.selectedClassSections.flatMap(section => this.learningSituationForm.get('content_' + section._id)?.value || []),
                ...
                next: (plan) => {
                    if (plan) {
                        ...
                        this.router.navigate(['/unit-plans', plan._id]).then(() => {
                            this.sb.open('Tu unidad multigrado ha sido guardada!', 'Ok', { duration: 2500 });
                        });
                    }
                },
            });
