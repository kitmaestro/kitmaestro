...
@Component({
  ...
  template: `
    <mat-card>
        <mat-card-header class="header">
            <h2 class="title" mat-card-title>Mis Evaluaciones Diagnósticas</h2>
            <a mat-flat-button color="primary" routerLink="/diagnostic-evaluation-generator">
                ...
                <table style="width: 100%; margin-bottom: 16px;">
                  ...
                  <tbody>
                    @for (eval of evaluations(); track eval._id) {
                      <tr>
                          <td>{{ eval.title }}</td>
                          <td>{{ eval.subject }} - {{ eval.schoolYear }}</td>
                          @if (eval.createdAt) {
                            <td>Creada el: {{ eval.createdAt | date:'medium' }}</td>
                          }
                          <td>
                            <div style="display: flex; justify-content: flex-end; gap: 8px;">
                              <button mat-icon-button color="primary" [routerLink]="['/diagnostic-evaluations', eval._id]" matTooltip="Ver Evaluación">
                                  <mat-icon>open_in_new</mat-icon>
                              </button>
                              <button mat-icon-button color="warn" (click)="deleteEvaluation(eval._id!)" matTooltip="Eliminar Evaluación">
  ...
  `,
  styles: `
    .evaluations-list, .no-evaluations {
      padding: 16px 0;
    }
...
export class DiagnosticEvaluationsComponent {
  ...
  loadEvaluations() {
    ...
    this.diagnosticEvaluationService.findAll({ user: this.userSettings()?._id }).subscribe({
      next: (evaluations) => {
  ...
  deleteEvaluation(id: string) {
    ...
    this.diagnosticEvaluationService.delete(id).subscribe({
      next: () => {
        this.evaluations.set(this.evaluations().filter(e => e._id !== id));
      },
