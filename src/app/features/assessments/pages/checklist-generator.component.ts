...
import { askGemini, createChecklist, createChecklistSuccess, loadBlocks, loadEntries, loadSections, loadSectionsSuccess, loadSubjectConceptLists, selectAiIsGenerating, selectAiResult, selectAiSerializedResult, selectAllClassSections, selectAllCompetenceEntries, selectAllContentBlocks, selectAuthUser } from '../../../store'
		...
		<div>
			<div>
				<div style="align-items: center; justify-content: space-between; display: flex">
					...
					<a mat-button routerLink="/assessments/checklists">Mis Listas de Cotejo</a>
				</div>
				<div>
...
export class ChecklistGeneratorComponent implements OnInit {
	...
	constructor() {
		...
		effect(() => {
			...
			if (contentBlocks && contentBlocks.length) {
				if (this.checklist) this.checklist.contentBlock = contentBlocks[0]
			}
		})
		effect(() => {
			const response = this.aiResponse()
			if (!response) return
			const arr: string[] = JSON.parse(response.slice(response.indexOf('['), response.lastIndexOf(']') + 1))
			if (arr && arr.length)
				this.checklist = this.fillChecklist(arr.filter((s) => !s.includes('*')))
		})
	}

	ngOnInit() {
		this.#store.dispatch(loadSections())
		this.#actions$.pipe(ofType(loadSectionsSuccess), takeUntil(this.#destroy$)).subscribe(({ sections }) => {
			if (sections.length) {
				this.checklistForm
					.get('section')
	...
	fillChecklist(criteria: string[]): Checklist {
		...
		const { title, activity, activityType } = this.checklistForm.getRawValue()
	...
	save() {
		if (this.checklist) {
			...
			this.#actions$.pipe(ofType(createChecklistSuccess), takeUntil(this.#destroy$)).subscribe(({ checklist }) => {
				this.router.navigateByUrl('/assessments/checklists/' + checklist._id)
			})
		}
	}
	...
	generate() {
		...
		const competenceStr = this.competence().flatMap(c => c.entries).join('\n- ')
		const contentBlockStr = this.contentBlocks().flatMap(b => b.achievement_indicators).join('\n -')
