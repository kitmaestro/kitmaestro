...
import { selectCurrentSystem, selectIsDeleting, selectIsDownloading } from '../../../store/score-systems/score-systems.selectors'
import { selectSectionStudents } from '../../../store/students/students.selectors'
import { deleteSystem, deleteSystemSuccess, downloadSystem, loadCurrentSubscription, loadStudentsBySection, loadSystem, loadSystemFailed } from '../../../store'
...
export class ScoreSystemDetailComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.#actions$.pipe(ofType(loadSystemFailed), take(1), takeUntil(this.#destroy$)).subscribe(() => {
	...
	deleteScoreSystem() {
		...
		this.#actions$.pipe(ofType(deleteSystemSuccess), take(1), takeUntil(this.#destroy$)).subscribe(() => {
			this.router.navigateByUrl('/assessments/grading-systems')
		})
	}
