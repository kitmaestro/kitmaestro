...
import { askGemini, createSystem, createSystemSuccess, loadBlocks, loadEntries, loadSections, loadSectionsSuccess, loadSectionSuccess, ScoreSystemDto, selectAiIsGenerating, selectAiSerializedResult, selectAllClassSections, selectAllCompetenceEntries, selectAllContentBlocks, selectAuthUser } from '../../../store'
...
export class ScoreSystemGeneratorComponent implements OnInit {
	...
	contentBlock = computed(() => this.contentBlocks().filter((block) => (this.form.get('content')?.value || []).includes(block.title)))
	...
	ngOnInit() {
		...
		this.#actions$.pipe(ofType(loadSectionsSuccess), takeUntil(this.#destroy$)).subscribe(({ sections }) => {
			if (!sections.length) {
				this.sb.open('No tienes sesiones todavia. Primero crea una para poder usar esta herramienta.', 'Ok')
			}
		})
	}
	...
	onSubmit() {
		...
		const competenceOptions = uniq<string>(this.competence().map((c) => c.name))
		const blocks = this.contentBlocks().filter(block => concepts.includes(block.title)).flatMap((block) => block.concepts)
		if (!section) return
...
Algunos puntos 'fijos' son los cuadernos (5 a 15 puntos) y la participacion activa en clase (de 5 a 10 puntos), autoevaluaciones/coevaluaciones (no mas de 5 puntos) el resto lo dejo en tus manos. Response solo con el json, no hagas comentarios, por favor.`
	...
	saveSystem() {
		if (this.scoreSystem) {
			...
			const system: ScoreSystemDto = { ...this.scoreSystem, content } as any
			this.#store.dispatch(createSystem({ system }))
			this.#actions$.pipe(
				ofType(createSystemSuccess),
				take(1),
				takeUntil(this.#destroy$),
			).subscribe(({ system }) => this.router.navigateByUrl('/assessments/grading-systems/' + system._id))
		}
	}
	...
	onSubjectChange(event: any) {
		...
		if (section) {
			...
			this.#store.dispatch(loadBlocks({ filters: { grade, subject, level } }))
		}
	}
	...
	onConceptChange(event: any) {
		...
		if (section) {
			...
			this.#store.dispatch(loadEntries({ filters: { grade: year, level, subject }}))
		}
	}
}
