...
import { askGemini, askGeminiFailure, loadSections, selectAiIsGenerating, selectAiResult, selectAllClassSections, selectAuthUser, selectIsLoadingSections } from '../../../store'
			...
			<div>
				...
				<div>
					<div style="margin-top: 24px">
						<form [formGroup]="improvementPlanForm" (ngSubmit)="onSubmit()">
							<div class="grid-2">
								<div>
							...
							<div>
								<mat-form-field appearance="outline">
									<mat-label>Áreas de mejora identificadas (Una por línea)</mat-label>
									<textarea
										matInput
							...
							<div>
								<mat-form-field appearance="outline">
									<mat-label>Fortalezas del estudiante/grupo (Una por línea)</mat-label>
									<textarea
										matInput
							...
							<div class="grid-2">
								<div>
									<mat-form-field appearance="outline">
										<mat-label>Duración del plan (semanas)</mat-label>
										<input
											type="number"
											matInput
							...
							<div style="display: flex; gap: 12px; justify-content: flex-end">
								...
								<button
									mat-button
									[disabled]="improvementPlanForm.invalid || generating()"
									type="submit"
								>
...
export class ImprovementPlanGeneratorComponent implements OnInit {
    ...
    improvementPlanForm = this.fb.group({
        ...
        duration: [4, [Validators.required, Validators.min(1), Validators.max(12)]],
...
**Duración:** ${formData.duration} semanas`
        ...
        this.#actions$.pipe(
            ...
            this.sb.open(e.error || 'Ha ocurrido un error al generar el plan de mejora.', 'Ok', { duration: 2500 })
        })
    }
    ...
    async downloadDocx(): Promise<void> {
        ...
        if (!result) {
            this.sb.open('No hay plan de mejora generado para descargar.', 'Ok', { duration: 2500 });
            return;
        }
        ...
        try {
            ...
            const fileName = `plan-mejora-${subject}-${section?.name || 'grado'}.docx`.replace(/\s+/g, '-');
            saveAs(blob, fileName);
            this.sb.open('Plan de mejora descargado exitosamente.', 'Ok', { duration: 2500 });
        } catch (error) {
            console.error('Error al generar el documento DOCX:', error);
            this.sb.open('Error al generar el documento.', 'Ok', { duration: 5000 });
        }
    }
