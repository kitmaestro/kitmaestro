...
import { DiagnosticEvaluationService, UserSettingsService } from '../../core/services';
import { ClassSection, GeneratedEvaluation, UserSettings } from '../../core/interfaces';
  ...
  template: `
    @if (evaluation(); as evaluation) {
        ...
        <header class="evaluation-header">
          ...
          <h2>
              ...
              @if (classSection(); as section) {
                  para {{ section.year | pretify }} de {{ section.level | pretify }}
              }
          </h2>
      ...
      <div class="student-info">
          <p><strong>Nombre:</strong> ___________________________________</p>
      ...
      <section class="evaluation-section">
          <h3>{{ section.title }}</h3>
          <p class="instructions"><em>{{ section.instructions }}</em></p>
  
          <ol class="question-list">
              @for (question of section.questions; track question.stem) {
              <li>
        ...
        <mat-card-header>
          <div style="width: 100%; display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
            ...
            <div style="display: flex; gap: 8px;">
              <button mat-icon-button routerLink="/diagnostic-evaluations" matTooltip="Volver al listado"><mat-icon>chevron_left</mat-icon></button>
              <button mat-icon-button (click)="deleteEvaluation(evaluation._id || '')" matTooltip="Eliminar Evaluación"><mat-icon>delete</mat-icon></button>
              <button mat-icon-button (click)="downloadDocx()" matTooltip="Descargar Evaluación"><mat-icon>download</mat-icon></button>
            </div>
          </div>
        </mat-card-header>
          <mat-card-content>
            <div style="padding: 1.25in; background-color: #fff; min-width: 8.5in margin-top: 24px;">
            <header class="evaluation-header">
              <h1>{{ evaluation.title }}</h1>
              <h2>
                  {{ evaluation.subject }} para {{ evaluation.year | pretify }} de {{ evaluation.level | pretify }}
              </h2>
              <p>{{ evaluation.schoolYear }}</p>
              @if (userSettings(); as user) {
                  <p>{{ user.title }}. {{ user.firstname }} {{ user.lastname }}</p>
...
export class DiagnosticEvaluationDetailComponent {
  ...
  deleteEvaluation(id: string) {
    ...
    this.diagnosticEvaluationService.delete(id).subscribe({
      next: () => {
        ...
        this.sb.open('Evaluación eliminada exitosamente', 'Cerrar', { duration: 3000 });
      },
  ...
  async downloadDocx(): Promise<void> {
    ...
    const paragraphs: Paragraph[] = [
      new Paragraph({
        children: [new TextRun({ text: evaluation.title, bold: true, size: 32 })],
        heading: HeadingLevel.TITLE,
        alignment: AlignmentType.CENTER,
      }),
      new Paragraph({
        children: [new TextRun({ text: evaluation.subject, bold: true, size: 28 })],
        heading: HeadingLevel.HEADING_1,
        alignment: AlignmentType.CENTER,
      }),
      new Paragraph({
        children: [new TextRun({ text: evaluation.schoolYear, size: 24 })],
        alignment: AlignmentType.CENTER,
        spacing: { after: 400 },
      }),
      new Paragraph({
