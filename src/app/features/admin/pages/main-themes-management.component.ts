...
@Component({
			...
			<mat-card-content>
						...
						<mat-panel-description>
							{{
								showCreateForm()
									...
									: 'Haz clic para añadir o editar un tema'
							}}
						</mat-panel-description>
					</mat-expansion-panel-header>

					<form
						[formGroup]="mainThemeForm"
						(ngSubmit)="onSubmit()"
						class="create-form"
					>
						<div class="form-grid">
							...
							>
								...
								/>
								@if (
									getFormControl('category')?.invalid &&
									getFormControl('category')?.touched
								) {
									<mat-error
										>La categoría es requerida.</mat-error
									>
								}
							</mat-form-field>
						...
						<div class="textarea-grid">
							...
							>
								...
								<textarea
									matInput
									cdkTextareaAutosize
									formControlName="topicsStr"
									rows="5"
									required
								></textarea>
								@if (
									getFormControl('topicsStr')?.invalid &&
									getFormControl('topicsStr')?.touched
								) {
									<mat-error
										>Al menos un tópico es
										requerido.</mat-error
									>
								}
							</mat-form-field>
						...
						<div class="form-actions">
							...
							<button
								...
								type="submit"
								[disabled]="
									mainThemeForm.invalid || isSubmitting()
								"
							>
								@if (isSubmitting()) {
									<mat-spinner
				...
				@if (isLoadingThemes()) {
					...
					<div class="table-container mat-elevation-z4">
						<table
							mat-table
							[dataSource]="filteredMainThemes()"
							class="main-themes-table"
						>
							...
							<ng-container matColumnDef="level">
								...
								<td mat-cell *matCellDef="let theme">
									{{ getDisplayValue(levels, theme.level) }}
								</td>
							</ng-container>
							<ng-container matColumnDef="year">
								...
								<td mat-cell *matCellDef="let theme">
									{{ getDisplayValue(years, theme.year) }}
								</td>
							</ng-container>
							<ng-container matColumnDef="subject">
								...
								<td mat-cell *matCellDef="let theme">
									{{
										getDisplayValue(subjects, theme.subject)
									}}
								</td>
							</ng-container>
							<ng-container matColumnDef="topics">
								...
								<td mat-cell *matCellDef="let theme">
									<mat-chip-listbox aria-label="Tópicos">
										@for (
											topic of theme.topics.slice(0, 3);
											track topic
										) {
	...
	styles: [
		`
			...
			.mat-column-topics .mat-mdc-chip-set {
				flex-wrap: nowrap;
			}
		`,
	],
...
export class MainThemesManagementComponent implements OnInit, OnDestroy {
			...
			.pipe(
				...
				catchError((error) => {
					...
					return this.#handleError(
						error,
						`Error al ${action} el tema principal.`,
					);
				}),
	...
	onEditTheme(theme: MainTheme): void {
		...
		this.#snackBar.open(`Editando tema: "${theme.category}".`, 'Cerrar', {
			duration: 2000,
		});
	}
	...
	onDeleteTheme(themeId: string): void {
		if (
			!confirm(
				'¿Estás seguro de que deseas eliminar este tema principal?',
			)
		) {
			return;
		}
		...
		this.#mainThemeService
			...
			.pipe(
				...
				catchError((error) =>
					this.#handleError(error, 'Error al eliminar el tema.'),
				),
				finalize(() => this.isLoadingThemes.set(false)),
			)
			.subscribe();
