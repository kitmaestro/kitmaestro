...
<mat-card class="competence-entries-card">
  <mat-card-header>
    ...
    <mat-card-subtitle>Administra las entradas de competencias y sus criterios</mat-card-subtitle>
  </mat-card-header>
  ...
  <mat-card-content>
    <mat-expansion-panel [expanded]="showCreateForm()" (opened)="showCreateForm.set(true)" (closed)="onExpansionPanelClosed()">
      <mat-expansion-panel-header>
        <mat-panel-title>
          {{ editingCompetenceId() ? 'Editar Competencia' : 'Crear Nueva Competencia' }}
        </mat-panel-title>
        <mat-panel-description>
          {{ showCreateForm() ? 'Cerrar para ver la lista' : 'Haz clic para añadir o editar una competencia' }}
        </mat-panel-description>
      </mat-expansion-panel-header>

      <form [formGroup]="competenceForm" (ngSubmit)="onSubmit()" class="create-form">
        <div class="form-grid">
          <mat-form-field appearance="outline" class="form-field">
            <mat-label>Nombre de la Competencia</mat-label>
            <input matInput formControlName="name" required>
            @if(getFormControl('name')?.invalid && getFormControl('name')?.touched){ <mat-error>El nombre es requerido.</mat-error> }
          </mat-form-field>
          ...
          <mat-form-field appearance="outline" class="form-field">
            ...
            <mat-select formControlName="grade" required>
             @for(grade of grades; track grade.value){ <mat-option [value]="grade.value">{{grade.viewValue}}</mat-option> }
            </mat-select>
            @if(getFormControl('grade')?.invalid && getFormControl('grade')?.touched){ <mat-error>Año/Grado es requerido.</mat-error> }
          </mat-form-field>
        ...
        <div class="textarea-grid">
          <mat-form-field appearance="outline" class="form-field textarea-field">
            ...
            <textarea matInput cdkTextareaAutosize formControlName="entriesStr" rows="5" required></textarea>
             @if(getFormControl('entriesStr')?.invalid && getFormControl('entriesStr')?.touched){ <mat-error>Al menos una entrada es requerida.</mat-error> }
          </mat-form-field>

           <mat-form-field appearance="outline" class="form-field textarea-field">
            <mat-label>Criterios de Evaluación (uno por línea)</mat-label>
            <textarea matInput cdkTextareaAutosize formControlName="criteriaStr" rows="5" required></textarea>
             @if(getFormControl('criteriaStr')?.invalid && getFormControl('criteriaStr')?.touched){ <mat-error>Al menos un criterio es requerido.</mat-error> }
          </mat-form-field>
        </div>

        <div class="form-actions">
          <button mat-stroked-button type="button" (click)="resetFormAndState()">Limpiar / Cancelar Edición</button>
          <button mat-raised-button color="primary" type="submit" [disabled]="competenceForm.invalid || isSubmitting()">
            @if(isSubmitting()){ <mat-spinner diameter="20" class="inline-spinner"></mat-spinner> {{ editingCompetenceId() ? 'Actualizando...' : 'Guardando...' }} }
            @else {
              <ng-container>
                <mat-icon>{{ editingCompetenceId() ? 'update' : 'save' }}</mat-icon>
                {{ editingCompetenceId() ? 'Actualizar Competencia' : 'Guardar Competencia' }}
              </ng-container>
            }
          </button>
        </div>
    ...
    <div class="filters-section">
      ...
      <form [formGroup]="filterForm" class="filter-form">
        <mat-form-field appearance="outline" class="filter-field">
          ...
          <input matInput formControlName="name" placeholder="Ej: Competencia Comunicativa">
        </mat-form-field>
        <mat-form-field appearance="outline" class="filter-field">
          <mat-label>Nivel</mat-label>
    ...
    @if(isLoadingCompetences()){
      <div class="loading-indicator"><mat-spinner diameter="50"></mat-spinner><p>Cargando competencias...</p></div>
    } @else if (filteredCompetences().length === 0) {
      <p class="no-results">No se encontraron competencias con los filtros aplicados.</p>
    } @else {
      <div class="table-container mat-elevation-z4">
        <table mat-table [dataSource]="filteredCompetences()" class="competence-entries-table">
          ...
          <ng-container matColumnDef="level">
            ...
            <td mat-cell *matCellDef="let item"> {{getDisplayValue(levels, item.level)}} </td>
          </ng-container>
          <ng-container matColumnDef="grade">
            <th mat-header-cell *matHeaderCellDef> Año/Grado </th>
            <td mat-cell *matCellDef="let item"> {{getDisplayValue(grades, item.grade)}} </td>
          </ng-container>
          <ng-container matColumnDef="subject">
            <th mat-header-cell *matHeaderCellDef> Asignatura </th>
            <td mat-cell *matCellDef="let item"> {{getDisplayValue(subjects, item.subject)}} </td>
          </ng-container>
          <ng-container matColumnDef="actions">
            <th mat-header-cell *matHeaderCellDef> Acciones </th>
            <td mat-cell *matCellDef="let item">
              <button mat-icon-button color="primary" (click)="onEditCompetence(item)" matTooltip="Editar Competencia">
                <mat-icon>edit</mat-icon>
              </button>
              <button mat-icon-button color="warn" (click)="onDeleteCompetence(item._id)" matTooltip="Eliminar Competencia">
  ...
  `,
    ...
    styles: [`
    :host { display: block; }
    .competence-entries-card { margin: 0 auto; padding: 15px 25px 25px 25px; }
    ...
    .competence-entries-table { width: 100%; }
...
export class CompetenceEntriesManagementComponent implements OnInit, OnDestroy {
    ...
    // --- Table ---
    displayedColumns: string[] = ['name', 'level', 'grade', 'subject', 'actions'];
    ...
    #loadCompetences(): void {
        ...
        this.#competenceService.findAll().pipe(
            ...
            tap(competences => {
                ...
                this.#applyClientSideFilters(this.filterForm.value, competences || []);
            }),
            catchError(error => this.#handleError(error, 'Error al cargar las competencias.')),
    ...
    #listenForFilterChanges(): void {
        this.filterForm.valueChanges.pipe(
            ...
            tap(filterValues => {
                this.#applyClientSideFilters(filterValues, this.allCompetences());
            })
        ).subscribe();
    }
    ...
    #applyClientSideFilters(filters: any, source: CompetenceEntry[]): void {
        let items = [...source];
        if (filters.name) {
            items = items.filter(i => i.name.toLowerCase().includes(filters.name.toLowerCase()));
        }
        if (filters.level) {
    ...
    resetFormAndState(): void {
        this.competenceForm.reset({
            name: '', level: '', grade: '', subject: '', entriesStr: '', criteriaStr: ''
        });
    ...
    onSubmit(): void {
        ...
        if (editingId) {
            operation$ = this.#competenceService.update(editingId, competenceData);
        } else {
            operation$ = this.#competenceService.createCompetence(competenceData as CompetenceEntry);
        }
        ...
        operation$.pipe(
            ...
            catchError(error => {
                ...
                return this.#handleError(error, `Error al ${action} la competencia.`);
            }),
    ...
    onEditCompetence(competence: CompetenceEntry): void {
        ...
        this.#snackBar.open(`Editando competencia: "${competence.name}".`, 'Cerrar', { duration: 2000 });
    }
    ...
    onDeleteCompetence(id: string): void {
        ...
        this.#competenceService.delete(id).pipe(
            ...
            catchError(error => this.#handleError(error, 'Error al eliminar la competencia.')),
            finalize(() => this.isLoadingCompetences.set(false))
        ).subscribe();
    }
}
