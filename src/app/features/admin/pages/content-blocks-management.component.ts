...
<mat-card class="content-blocks-card">
  <mat-card-header>
    ...
    <mat-card-subtitle>Administra los bloques de contenido curricular</mat-card-subtitle>
  </mat-card-header>
  ...
  <mat-card-content>
    <mat-expansion-panel [expanded]="showCreateForm()" (opened)="showCreateForm.set(true)" (closed)="onExpansionPanelClosed()">
      <mat-expansion-panel-header>
        <mat-panel-title>
          {{ editingBlockId() ? 'Editar Bloque de Contenido' : 'Crear Nuevo Bloque de Contenido' }}
        </mat-panel-title>
        <mat-panel-description>
          {{ showCreateForm() ? 'Cerrar para ver la lista' : 'Haz clic para añadir o editar un bloque' }}
        </mat-panel-description>
      </mat-expansion-panel-header>

      <form [formGroup]="contentBlockForm" (ngSubmit)="onSubmit()" class="create-form">
        <div class="form-grid">
          <mat-form-field appearance="outline" class="form-field">
            <mat-label>Título</mat-label>
            <input matInput formControlName="title" required>
            @if(getFormControl('title')?.invalid && getFormControl('title')?.touched){ <mat-error>Título es requerido.</mat-error> }
          </mat-form-field>
          ...
          <mat-form-field appearance="outline" class="form-field">
            ...
            <mat-select formControlName="year" required>
             @for(year of createFormYears(); track year.value){ <mat-option [value]="year.value">{{year.viewValue}}</mat-option> }
            </mat-select>
          ...
          <mat-form-field appearance="outline" class="form-field">
            ...
            @if(getFormControl('order')?.invalid && getFormControl('order')?.touched){ <mat-error>Orden es requerido (mínimo 1).</mat-error> }
          </mat-form-field>
        </div>
        ...
        <div class="textarea-grid">
          <mat-form-field appearance="outline" class="form-field textarea-field">
            ...
            <textarea matInput cdkTextareaAutosize formControlName="conceptsStr" rows="3"></textarea>
          </mat-form-field>

          <mat-form-field appearance="outline" class="form-field textarea-field">
            <mat-label>Procedimientos (uno por línea)</mat-label>
            <textarea matInput cdkTextareaAutosize formControlName="proceduresStr" rows="3"></textarea>
          </mat-form-field>

          <mat-form-field appearance="outline" class="form-field textarea-field">
            <mat-label>Actitudes y Valores (uno por línea)</mat-label>
            <textarea matInput cdkTextareaAutosize formControlName="attitudesStr" rows="3"></textarea>
          </mat-form-field>

          <mat-form-field appearance="outline" class="form-field textarea-field">
            <mat-label>Indicadores de Logro (uno por línea)</mat-label>
            <textarea matInput cdkTextareaAutosize formControlName="achievementIndicatorsStr" rows="3"></textarea>
          </mat-form-field>
        </div>

        <div class="form-actions">
          <button mat-stroked-button type="button" (click)="resetFormAndState()">Limpiar / Cancelar Edición</button>
          <button mat-raised-button color="primary" type="submit" [disabled]="contentBlockForm.invalid || isSubmitting()">
            @if(isSubmitting()){ <mat-spinner diameter="20" class="inline-spinner"></mat-spinner> {{ editingBlockId() ? 'Actualizando...' : 'Guardando...' }} }
            @else {
              <ng-container>
                <mat-icon>{{ editingBlockId() ? 'update' : 'save' }}</mat-icon>
                {{ editingBlockId() ? 'Actualizar Bloque' : 'Guardar Bloque' }}
              </ng-container>
            }
          </button>
        </div>
    ...
    <div class="filters-section">
      ...
      <form [formGroup]="filterForm" class="filter-form">
        <mat-form-field appearance="outline" class="filter-field">
          ...
          <input matInput formControlName="title" placeholder="Ej: Los Ecosistemas">
        </mat-form-field>
        <mat-form-field appearance="outline" class="filter-field">
          <mat-label>Nivel</mat-label>
        ...
        <mat-form-field appearance="outline" class="filter-field">
          ...
          <mat-select formControlName="year">
            <mat-option value="">Todos</mat-option>
             @for(year of filterYearsForFilter(); track year.value){ <mat-option [value]="year.value">{{year.viewValue}}</mat-option> }
          </mat-select>
        </mat-form-field>
        <mat-form-field appearance="outline" class="filter-field">
          <mat-label>Asignatura</mat-label>
    ...
    @if(isLoadingBlocks()){
      <div class="loading-indicator"><mat-spinner diameter="50"></mat-spinner><p>Cargando bloques...</p></div>
    } @else if (filteredContentBlocks().length === 0) {
      <p class="no-results">No se encontraron bloques de contenido con los filtros aplicados.</p>
    } @else {
      <div class="table-container mat-elevation-z4">
        <table mat-table [dataSource]="filteredContentBlocks()" class="content-blocks-table">
          <ng-container matColumnDef="title">
            <th mat-header-cell *matHeaderCellDef> Título </th>
            <td mat-cell *matCellDef="let block"> {{block.title}} </td>
          </ng-container>
          <ng-container matColumnDef="level">
            <th mat-header-cell *matHeaderCellDef> Nivel </th>
            <td mat-cell *matCellDef="let block"> {{getDisplayValue(levels, block.level)}} </td>
          </ng-container>
          <ng-container matColumnDef="year">
            <th mat-header-cell *matHeaderCellDef> Año/Grado </th>
            <td mat-cell *matCellDef="let block"> {{getDisplayValue(allYears, block.year)}} </td>
          </ng-container>
          <ng-container matColumnDef="subject">
            <th mat-header-cell *matHeaderCellDef> Asignatura </th>
            <td mat-cell *matCellDef="let block"> {{getDisplayValue(subjects, block.subject)}} </td>
          </ng-container>
          <ng-container matColumnDef="order"> <th mat-header-cell *matHeaderCellDef> Orden </th>
            <td mat-cell *matCellDef="let block"> {{block.order}} </td>
          </ng-container>
          <ng-container matColumnDef="actions">
            <th mat-header-cell *matHeaderCellDef> Acciones </th>
            <td mat-cell *matCellDef="let block">
              <button mat-icon-button color="primary" (click)="onEditBlock(block)" matTooltip="Editar Bloque">
                <mat-icon>edit</mat-icon>
              </button>
              <button mat-icon-button color="warn" (click)="onDeleteBlock(block._id)" matTooltip="Eliminar Bloque">
  ...
  styles: [`
    :host { display: block; }
    .content-blocks-card { margin: 0 auto; padding: 15px 25px 25px 25px; }
    .create-form, .filter-form { display: flex; flex-direction: column; gap: 10px; margin-bottom: 20px; }
    .form-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px; }
    .textarea-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 15px; margin-top: 15px; }
    .form-field { width: 100%; }
    .textarea-field textarea { min-height: 80px; }
    ...
    .filter-form { display: flex; flex-direction: row; flex-wrap: wrap; align-items: center; }
    .filter-field { margin-right: 15px; margin-bottom: 10px; min-width: 200px; }
    ...
    .content-blocks-table { width: 100%; }
    .mat-column-actions { width: 120px; text-align: right; }
    .mat-mdc-form-field { font-size: 0.9em; }
  `],
...
export class ContentBlocksManagementComponent implements OnInit, OnDestroy {
  ...
  filterYearsForFilter = computed(() => { // Renamed to avoid confusion
  ...
  // --- Table ---
  displayedColumns: string[] = ['title', 'level', 'year', 'subject', 'order', 'actions']; // Added 'order'
  ...
  #loadContentBlocks(filters?: any): void {
    ...
    this.#contentBlockService.findAll(filters).pipe(
      ...
      catchError(error => this.#handleError(error, 'Error al cargar los bloques de contenido.')),
  ...
  #listenForFilterChanges(): void {
    this.filterForm.valueChanges.pipe(
      ...
      tap(filterValues => {
        ...
        if (filterValues.title) activeFilters.title = filterValues.title;
        if (filterValues.level) activeFilters.level = filterValues.level;
        if (filterValues.year) activeFilters.year = filterValues.year;
        if (filterValues.subject) activeFilters.subject = filterValues.subject;
        // Client-side filtering based on allContentBlocks
        this.#applyClientSideFilters(activeFilters, this.allContentBlocks());
      })
    ).subscribe();
  }
  ...
  #applyClientSideFilters(filters: any, sourceBlocks: ContentBlock[]): void {
      ...
      if (filters.title) {
          blocks = blocks.filter(b => b.title.toLowerCase().includes(filters.title.toLowerCase()));
      }
      if (filters.level) {
  ...
  resetFormAndState(): void { // Renamed for clarity
    this.contentBlockForm.reset({
      title: '', level: '', year: '', subject: '', order: 1,
      conceptsStr: '', proceduresStr: '', attitudesStr: '', achievementIndicatorsStr: ''
    });
  ...
  onSubmit(): void { // Renamed from onCreateSubmit
    ...
    const blockData: Partial<ContentBlock> = {
      ...
      achievement_indicators: this.#splitStringToArray(formValue.achievementIndicatorsStr),
    };

    let operation$: Observable<ContentBlock | ApiUpdateResponse>;

    if (this.editingBlockId()) {
      operation$ = this.#contentBlockService.update(this.editingBlockId()!, blockData);
    } else {
      operation$ = this.#contentBlockService.create(blockData as ContentBlock);
    }
    ...
    operation$.pipe(
      ...
      tap(() => {
        ...
        if (this.editingBlockId()) {
            ...
            this.#snackBar.open(`Bloque "${title}" creado exitosamente.`, 'Cerrar', { duration: 3000 });
        }
        this.resetFormAndState();
        this.showCreateForm.set(false);
      ...
      catchError(error => {
          ...
          const action = this.editingBlockId() ? 'actualizar' : 'crear';
          return this.#handleError(error, `Error al ${action} el bloque de contenido.`);
      }),
  ...
  onEditBlock(block: ContentBlock): void {
      ...
      this.contentBlockForm.patchValue({
          ...
          achievementIndicatorsStr: this.#joinArrayToString(block.achievement_indicators),
      });
      this.showCreateForm.set(true); // Open form for editing
      this.#snackBar.open(`Editando bloque: "${block.title}".`, 'Cerrar', { duration: 2000 });
  }
  ...
  onDeleteBlock(blockId: string): void {
      ...
      this.isLoadingBlocks.set(true);
      this.#contentBlockService.delete(blockId).pipe(
          takeUntil(this.#destroy$),
          tap(response => {
          ...
          catchError(error => {
              ...
              return this.#handleError(error, 'Error al eliminar el bloque.');
          }),
          finalize(() => this.isLoadingBlocks.set(false))
      ).subscribe();
  }
