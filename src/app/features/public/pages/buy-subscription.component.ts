...
export class BuySubscriptionComponent implements OnInit {
	...
	pricingPlans = [
		{
			...
			price: 0,
			description:
				'Herramientas esenciales para empezar a organizar tus clases.',
			features: [
		...
		},
		{
			...
			price: 9.58,
			description:
				'Ideal para docentes de área que buscan simplificar su vida, ahorrando al menos 5 horas a la semana.',
			features: [
		...
		},
		{
			...
			price: 15.97,
			description:
				'Perfecto para docentes en aula, que imparten 4 o más asignaturas en uno o varios grados, logrando ahorras al menos 10 horas de trabajo cada semana.',
			features: [
		...
		},
		{
			...
			price: 38.36,
			description:
				'Perfecto para el docente eficiente e innovador que valora su tiempo al máximo, con este plan, el maestro puede ahorrar hasta 20 horas a la semana.',
			features: [
	...
	renderButtons() {
		...
		this.pricingPlans
			.filter((p) => p.price)
			.forEach(({ id: plan_id, name, code, price }) => {
				const days = 30;
				paypal
					.Buttons({
						style,
	...
	goFree() {
		this.userSubscriptionService
			.subscribe('FREE', 'none', 0, 0)
			.subscribe(() => this.alertSuccess('Gratuito'));
	}
	...
	alertSuccess(plan: string) {
		this.router
			.navigate(plan == 'Gratuito' ? ['/'] : ['/premium-welcome'], {
