import {
  Component, signal, inject, ChangeDetectionStrategy, OnInit, OnDestroy, ViewEncapsulation, computed
} from '@angular/core';
import { FormBuilder, ReactiveFormsModule, Validators, AbstractControl } from '@angular/forms';
import { CommonModule } from '@angular/common';
import { Subject, Observable, firstValueFrom, takeUntil, tap, catchError, EMPTY, finalize, switchMap, filter, map, distinctUntilChanged } from 'rxjs';
...
import { SubjectConceptListService } from '../../core/services/subject-concept-list.service'; // Correct path
...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="math-problem-card"> <mat-card-header>
    <mat-card-title>Generador de Problemas Matemáticos</mat-card-title>
    <mat-card-subtitle>Crea problemas contextualizados para tus clases de matemáticas</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="mathProblemForm" (ngSubmit)="onSubmit()" class="math-problem-form">
        <div class="form-row">
          ...
          <mat-form-field appearance="outline" class="form-field">
            ...
            <mat-select formControlName="topicConcept" required>
               @if(isLoadingTopics()){
                <mat-option disabled><mat-spinner diameter="20" class="inline-spinner"></mat-spinner> Cargando temas...</mat-option>
              } @else {
                 @for (topic of availableTopics(); track topic) {
                  <mat-option [value]="topic">{{ topic }}</mat-option>
                }
                 @if(!availableTopics().length && sectionCtrl?.valid && !isLoadingTopics()){
                   <mat-option disabled>No hay temas para esta sección.</mat-option>
                   <mat-option [value]="'Otro'">{{ 'Otro' }}</mat-option> } @else if (availableTopics().length > 1 || !isLoadingTopics()) {
             ...
             @if (topicConceptCtrl?.invalid && topicConceptCtrl?.touched) {
              <mat-error>Selecciona o escribe un tema.</mat-error>
            }
          </mat-form-field>
        </div>
        ...
        <div class="form-row">
          @if (topicConceptCtrl?.value === 'Otro') {
            <mat-form-field appearance="outline" class="form-field full-width-field">
              <mat-label>Escribe el tema matemático personalizado</mat-label>
              <input matInput formControlName="customTopic" required placeholder="Ej: Regla de tres simple, Teorema de Pitágoras">
               @if (customTopicCtrl?.invalid && customTopicCtrl?.touched) {
                <mat-error>Ingresa el tema personalizado.</mat-error>
              }
            </mat-form-field>
          }
        </div>
        ...
        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="mathProblemForm.invalid || isGenerating()">
            @if (isGenerating()) {
              ...
              <ng-container>
                 <mat-icon>functions</mat-icon> Generar Problemas
              </ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="math-problem-result">
        ...
        <div class="math-problem-result-content" [innerHTML]="generatedProblems().replaceAll('\n', '<br>')"></div>

        <div class="result-actions">
          <button mat-stroked-button color="primary" (click)="goBack()">
            <mat-icon>arrow_back</mat-icon> Volver
          </button>
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedProblems() || generatedProblems().startsWith('Ocurrió un error')">
  ...
  styles: [`
    :host { display: block; }
    .math-problem-card { margin: 0 auto; padding: 15px 25px 25px 25px; }
    .math-problem-form { margin-top: 16px; display: flex; flex-direction: column; gap: 15px; }
    ...
    .math-problem-result { margin-top: 20px; }
    .math-problem-result h3 { margin-bottom: 15px; }
    ...
    .math-problem-result-content p[style*="text-indent"] { /* Example for indented paragraphs */
        margin-left: 20px;
    }
...
export class MathProblemGeneratorComponent implements OnInit, OnDestroy {
  ...
  #loadMathTopics(sectionId: string): void {
      ...
      this.#conceptService.findAll(filterParams).pipe(
        takeUntil(this.#destroy$),
        map(conceptLists => {
          const concepts = conceptLists?.flatMap(list => list.concepts || []) || [];
          return [...new Set(concepts)]; // Unique concepts
        }),
        catchError(error => this.#handleError(error, 'Error al cargar los temas de matemática.')),
  ...
  #listenForSectionChanges(): void {
   ...
   #listenForTopicConceptChanges(): void {
     this.topicConceptCtrl?.valueChanges.pipe(
       takeUntil(this.#destroy$),
       distinctUntilChanged()
     ).subscribe(value => {
  ...
  async onSubmit(): Promise<void> {
    ...
    const topic = this.finalTopic(); // Use computed signal
     ...
     if (!topic) {
        this.#snackBar.open('Por favor, selecciona o introduce un tema matemático válido.', 'Cerrar', { duration: 3000 });
    ...
    try {
      ...
      this.generatedProblems.set(result?.response || 'No se pudieron generar los problemas.');
      this.showResult.set(true);
    } catch (error) {
       this.generatedProblems.set('Ocurrió un error al generar los problemas. Por favor, inténtalo de nuevo.');
  ...
  downloadDocx(): void {
    ...
    if (!problemsText || problemsText.startsWith('Ocurrió un error')) return;
    ...
    const doc = new Document({
       numbering: { // Define the numbering scheme
            config: [
                {
      ...
      sections: [
        {
          properties: {},
          children: [
              new Paragraph({ text: `Problemas Matemáticos`, heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 100 } }),
              new Paragraph({ text: `Tema: ${topic}`, heading: HeadingLevel.HEADING_2, alignment: AlignmentType.CENTER, spacing: { after: 300 } }),
  ...
  get sectionCtrl(): AbstractControl | null { return this.mathProblemForm.get('section'); }
  get topicConceptCtrl(): AbstractControl | null { return this.mathProblemForm.get('topicConcept'); }
  get customTopicCtrl(): AbstractControl | null { return this.mathProblemForm.get('customTopic'); }
}
