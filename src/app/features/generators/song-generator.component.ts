...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="song-generator-card"> <mat-card-header>
    <mat-card-title>Generador de Canciones</mat-card-title>
    <mat-card-subtitle>Crea letras de canciones originales para tus actividades</mat-card-subtitle>
  </mat-card-header>
  ...
  <mat-card-content>
      ...
      <form [formGroup]="songForm" (ngSubmit)="onSubmit()" class="song-form">
        ...
        <div class="form-row">
           <mat-form-field appearance="outline" class="form-field full-width-field">
            ...
            <input matInput formControlName="topic" required placeholder="Ej: Día de las Madres, La Amistad, Cuidar el Planeta">
             @if (topicCtrl?.invalid && topicCtrl?.touched) {
              <mat-error>La temática es requerida.</mat-error>
            }
          </mat-form-field>
        </div>

        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="songForm.invalid || isGenerating()">
            @if (isGenerating()) {
              ...
              <ng-container>
                 <mat-icon>music_note</mat-icon> Generar Canción
              </ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="song-result">
        ...
        <div class="song-result-content" [innerHTML]="generatedSong().replaceAll('\n', '<br>')"></div>

        <div class="result-actions">
          <button mat-stroked-button color="primary" (click)="goBack()">
            <mat-icon>arrow_back</mat-icon> Volver
          </button>
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedSong() || generatedSong().startsWith('Ocurrió un error')">
  ...
  styles: [`
    :host { display: block; }
    .song-generator-card { margin: 0 auto; padding: 15px 25px 25px 25px; }
    .song-form { margin-top: 16px; display: flex; flex-direction: column; gap: 15px; }
    ...
    .song-result { margin-top: 20px; }
    .song-result h3 { margin-bottom: 15px; }
    .song-result-content {
      ...
      line-height: 1.7;
      font-family: 'Verdana', Geneva, Tahoma, sans-serif; /* Clean font */
      font-size: 11pt;
      margin-bottom: 20px;
      max-width: 100%;
    ...
    /* Style potential chorus/verse markers if AI uses them */
     .song-result-content strong { /* Example: Bold text for [Coro] etc. */
        display: block;
        margin-top: 1em;
        margin-bottom: 0.5em;
        font-weight: bold;
...
export class SongGeneratorComponent implements OnInit, OnDestroy {
  ...
  readonly complexityLevels = ['Simple / Repetitiva', 'Media / Estructurada', 'Elaborada / Creativa'];
  ...
  #loadSections(): void {
   ...
   #getComplexityInstruction(complexitySelection: string): string {
       switch(complexitySelection) {
           case 'Simple / Repetitiva': return 'muy simple, con vocabulario básico, estructura repetitiva (verso-coro sencillo) y rimas fáciles, ideal para niños pequeños';
           case 'Media / Estructurada': return 'con una estructura clara (varias estrofas, coro definido), vocabulario estándar para la edad, rimas y ritmo agradables';
           case 'Elaborada / Creativa': return 'más elaborada, con lenguaje más creativo o poético, quizás una estructura menos predecible (puente musical?) y rimas más trabajadas';
           default: return 'con una estructura clara (varias estrofas, coro definido), vocabulario estándar para la edad, rimas y ritmo agradables';
       }
   }
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.generatedSong.set(result?.response || 'No se pudo generar la letra de la canción.');
      this.showResult.set(true);
    } catch (error) {
       this.generatedSong.set('Ocurrió un error al generar la canción. Por favor, inténtalo de nuevo.');
  ...
  downloadDocx(): void {
    ...
    const topicName = (formValue.topic || 'Cancion').substring(0,20).replace(/[^a-z0-9]/gi, '_');
    const complexityName = (formValue.complexity || 'Media').substring(0,10).replace(/[^a-z0-9]/gi, '_');
      ...
      sections: [
        {
          properties: {},
          children: [
              new Paragraph({ text: `Letra de Canción Generada`, heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 300 } }),
              new Paragraph({ text: `Temática: ${formValue.topic}`, alignment: AlignmentType.CENTER, style: "SubtleEmphasis"}),
  ...
  get sectionCtrl(): AbstractControl | null { return this.songForm.get('section'); }
  get complexityCtrl(): AbstractControl | null { return this.songForm.get('complexity'); }
  get topicCtrl(): AbstractControl | null { return this.songForm.get('topic'); }
}
