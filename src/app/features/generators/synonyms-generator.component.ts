...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="synonyms-generator-card"> <mat-card-header>
    <mat-card-title>Generador de Sinónimos</mat-card-title>
    <mat-card-subtitle>Crea listas de palabras y sus sinónimos contextualizados</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="synonymsForm" (ngSubmit)="onSubmit()" class="synonyms-form">
        <div class="form-row">
        ...
        <div class="form-row">
           <mat-form-field appearance="outline" class="form-field">
            ...
            <input matInput type="number" formControlName="quantity" required min="1" max="20">
             @if (quantityCtrl?.invalid && quantityCtrl?.touched) {
                ...
                @if(quantityCtrl?.hasError('max')){ <mat-error>Máximo 20 palabras.</mat-error> }
            }
          </mat-form-field>
         ...
         <div class="form-row">
           <mat-form-field appearance="outline" class="form-field full-width-field">
            ...
            <input matInput formControlName="topic" placeholder="Ej: El clima, Emociones, Herramientas">
          </mat-form-field>
        </div>


        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="synonymsForm.invalid || isGenerating()">
            @if (isGenerating()) {
              ...
              <ng-container>
                 <mat-icon>find_replace</mat-icon> Generar Sinónimos
              </ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="synonyms-result">
        ...
        <div class="synonyms-result-content" [innerHTML]="generatedSynonyms().replaceAll('\n', '<br>')">
          </div>
        ...
        <div class="result-actions">
          ...
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedSynonyms() || generatedSynonyms().startsWith('Ocurrió un error')">
  ...
  styles: [`
    :host { display: block; }
    .synonyms-generator-card { margin: 0 auto; padding: 15px 25px 25px 25px; }
    .synonyms-form { margin-top: 16px; display: flex; flex-direction: column; gap: 15px; }
    ...
    .synonyms-result { margin-top: 20px; }
    .synonyms-result h3 { margin-bottom: 15px; }
    .synonyms-result-content {
      ...
      font-family: 'Verdana', Geneva, Tahoma, sans-serif; /* Clean sans-serif */
      font-size: 11pt;
      margin-bottom: 20px;
      max-width: 100%;
     ...
     .synonyms-result-content strong { /* Example: Bold text for original word */
        display: inline-block; /* Or block if needed */
        margin-right: 5px;
        font-weight: bold;
     }
     .synonyms-result-content br + strong { /* Add space before new word */
        margin-top: 1em;
        display: block;
     }
...
export class SynonymsGeneratorComponent implements OnInit, OnDestroy {
  ...
  synonymsForm = this.#fb.group({
    ...
    quantity: [5, [Validators.required, Validators.min(1), Validators.max(20)]], // Default 5
  ...
  #listenForSectionChanges(): void {
   ...
   #getDifficultyInstruction(difficultySelection: string): string {
       switch(difficultySelection) {
           case 'Fácil': return 'palabras comunes y sinónimos muy directos y conocidos';
           case 'Medio': return 'palabras de uso estándar con una mezcla de sinónimos comunes y algunos menos frecuentes';
           case 'Difícil': return 'palabras menos comunes o más específicas, con sinónimos más variados y precisos, quizás incluyendo lenguaje figurado si aplica';
           default: return 'palabras de uso estándar con sinónimos de dificultad media';
       }
   }
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.generatedSynonyms.set(result?.response || 'No se pudieron generar los sinónimos.');
      this.showResult.set(true);
    } catch (error) {
       this.generatedSynonyms.set('Ocurrió un error al generar los sinónimos. Por favor, inténtalo de nuevo.');
  ...
  downloadDocx(): void {
    ...
    if (!synonymsText || synonymsText.startsWith('Ocurrió un error')) return;
    ...
    const subjectName = (this.#pretify(formValue.subject || 'Asignatura')).replace(/[^a-z0-9]/gi, '_');
    const topicName = (formValue.topic || 'Sinonimos').substring(0,15).replace(/[^a-z0-9]/gi, '_');
    ...
    wordBlocks.forEach((block, index) => {
        ...
        const synonymsLine = lines.find(l => l.trim().startsWith("Sinónimos:"));
        ...
        if (word) {
            paragraphs.push(new Paragraph({
                children: [new TextRun({ text: word, bold: true, size: 24 })], // Make word bold and slightly larger
                spacing: { before: index > 0 ? 240 : 0, after: 60 }, // Add space before new word
            }));
        }
        if (synonyms) {
             paragraphs.push(new Paragraph({
                children: [new TextRun({ text: synonyms, italics: true })], // Italicize synonyms
      ...
      sections: [
        {
          properties: {},
          children: [
              new Paragraph({ text: `Lista de Sinónimos`, heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 300 } }),
  ...
  get sectionCtrl(): AbstractControl | null { return this.synonymsForm.get('section'); }
  get subjectCtrl(): AbstractControl | null { return this.synonymsForm.get('subject'); }
  get quantityCtrl(): AbstractControl | null { return this.synonymsForm.get('quantity'); }
  get difficultyCtrl(): AbstractControl | null { return this.synonymsForm.get('difficulty'); }
  get topicCtrl(): AbstractControl | null { return this.synonymsForm.get('topic'); }
}
