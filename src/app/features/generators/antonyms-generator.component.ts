...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="antonyms-generator-card"> <mat-card-header>
    <mat-card-title>Generador de Antónimos</mat-card-title>
    <mat-card-subtitle>Crea listas de palabras y sus antónimos contextualizados</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="antonymsForm" (ngSubmit)="onSubmit()" class="antonyms-form">
        <div class="form-row">
        ...
        <div class="form-row">
         ...
         <div class="form-row">
           <mat-form-field appearance="outline" class="form-field full-width-field">
            ...
            <input matInput formControlName="topic" placeholder="Ej: Sentimientos, Tamaños, Acciones">
          </mat-form-field>
        </div>


        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="antonymsForm.invalid || isGenerating()">
            @if (isGenerating()) {
              ...
              <ng-container>
                 <mat-icon>compare_arrows</mat-icon> Generar Antónimos
              </ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="antonyms-result">
        ...
        <div class="antonyms-result-content" [innerHTML]="generatedAntonyms().replaceAll('\n', '<br>')">
          </div>
        ...
        <div class="result-actions">
          ...
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedAntonyms() || generatedAntonyms().startsWith('Ocurrió un error')">
  ...
  styles: [`
    :host { display: block; }
    .antonyms-generator-card { margin: 0 auto; padding: 15px 25px 25px 25px; }
    .antonyms-form { margin-top: 16px; display: flex; flex-direction: column; gap: 15px; }
    ...
    .antonyms-result { margin-top: 20px; }
    .antonyms-result h3 { margin-bottom: 15px; }
    .antonyms-result-content {
     ...
     .antonyms-result-content strong { /* Example: Bold text for original word */
        display: inline-block;
        margin-right: 5px;
        font-weight: bold;
     }
      .antonyms-result-content br + strong { /* Add space before new word */
        margin-top: 1em;
        display: block;
     }
...
export class AntonymsGeneratorComponent implements OnInit, OnDestroy {
  ...
  #listenForSectionChanges(): void {
   ...
   #getDifficultyInstruction(difficultySelection: string): string {
       switch(difficultySelection) {
           case 'Fácil': return 'palabras comunes con antónimos claros y directos';
           case 'Medio': return 'palabras de uso estándar con antónimos comunes, quizás algunos menos obvios';
           case 'Difícil': return 'palabras menos comunes o más abstractas, con antónimos que requieran mayor comprensión del matiz';
           default: return 'palabras de uso estándar con antónimos de dificultad media';
       }
   }
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.generatedAntonyms.set(result?.response || 'No se pudieron generar los antónimos.');
      this.showResult.set(true);
    } catch (error) {
       this.generatedAntonyms.set('Ocurrió un error al generar los antónimos. Por favor, inténtalo de nuevo.');
  ...
  downloadDocx(): void {
    ...
    if (!antonymsText || antonymsText.startsWith('Ocurrió un error')) return;
    ...
    const topicName = (formValue.topic || 'Antonimos').substring(0,15).replace(/[^a-z0-9]/gi, '_');
    ...
    wordBlocks.forEach((block, index) => {
        ...
        const antonymsLine = lines.find(l => l.trim().startsWith("Antónimos:")); // Changed from Sinónimos
        ...
        if (antonyms) {
             paragraphs.push(new Paragraph({
                children: [new TextRun({ text: antonyms, italics: true })], // Italicize antonyms
      ...
      sections: [
        {
          properties: {},
          children: [
              new Paragraph({ text: `Lista de Antónimos`, heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 300 } }),
  ...
  get sectionCtrl(): AbstractControl | null { return this.antonymsForm.get('section'); }
  get subjectCtrl(): AbstractControl | null { return this.antonymsForm.get('subject'); }
  get quantityCtrl(): AbstractControl | null { return this.antonymsForm.get('quantity'); }
  get difficultyCtrl(): AbstractControl | null { return this.antonymsForm.get('difficulty'); }
  get topicCtrl(): AbstractControl | null { return this.antonymsForm.get('topic'); }
}
