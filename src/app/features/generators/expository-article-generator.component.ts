...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="expository-article-card"> <mat-card-header>
    <mat-card-title>Generador de Artículo Expositivo</mat-card-title>
    <mat-card-subtitle>Crea ejemplos de artículos expositivos para tus clases</mat-card-subtitle>
  </mat-card-header>
  ...
  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="articleForm" (ngSubmit)="onSubmit()" class="article-form">
        ...
        <div class="form-row">
          <mat-form-field appearance="outline" class="form-field">
           ...
           <mat-form-field appearance="outline" class="form-field">
            ...
            <input matInput formControlName="topic" required placeholder="Ej: El sistema solar, Los volcanes, La fotosíntesis">
        ...
        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="articleForm.invalid || isGenerating()">
            @if (isGenerating()) {
              ...
              <ng-container>
                 <mat-icon>article</mat-icon> Generar Artículo
              </ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="article-result">
        ...
        <div class="article-result-content" [innerHTML]="generatedArticle().replaceAll('\n\n', '<br><br>').replaceAll('\n', '<br>')">
          </div>
        ...
        <div class="result-actions">
          ...
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedArticle() || generatedArticle().startsWith('Ocurrió un error')">
  ...
  styles: [`
    :host { display: block; }
    .expository-article-card { margin: 0 auto; padding: 15px 25px 25px 25px; }
    .article-form { margin-top: 16px; display: flex; flex-direction: column; gap: 15px; }
    ...
    .article-result { margin-top: 20px; }
    .article-result h3 { margin-bottom: 15px; }
...
export class ExpositoryArticleGeneratorComponent implements OnInit, OnDestroy {
  ...
  #loadSections(): void {
   ...
   #getComplexityInstruction(complexitySelection: string): string {
       switch(complexitySelection) {
           case 'Básica': return 'con una estructura simple (introducción corta, 1-2 párrafos de desarrollo con ideas principales, conclusión breve), frases cortas y directas';
           case 'Media': return 'con una estructura estándar (introducción, varios párrafos de desarrollo con algunos detalles o ejemplos, conclusión), frases de longitud variada';
           case 'Avanzada': return 'bien estructurado (introducción, desarrollo detallado en varios párrafos con explicaciones más profundas, ejemplos o datos, conclusión sólida), usando quizás frases más complejas';
           default: return 'con una estructura estándar (introducción, varios párrafos de desarrollo, conclusión)';
       }
   }
   ...
   #getVocabularyInstruction(vocabularySelection: string): string {
       switch(vocabularySelection) {
           case 'Reducido': return 'un vocabulario básico y común, evitando tecnicismos';
           case 'Medio': return 'un vocabulario estándar para la edad, explicando términos clave si es necesario';
           case 'Amplio': return 'un vocabulario más preciso y variado, incluyendo términos específicos del tema (explicándolos si son complejos)';
           default: return 'un vocabulario estándar para la edad';
       }
   }
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.generatedArticle.set(result?.response || 'No se pudo generar el artículo.');
      this.showResult.set(true);
    } catch (error) {
       this.generatedArticle.set('Ocurrió un error al generar el artículo. Por favor, inténtalo de nuevo.');
  ...
  downloadDocx(): void {
    ...
    const topicName = (formValue.topic || 'Articulo').substring(0,20).replace(/[^a-z0-9]/gi, '_');
    const complexityName = (formValue.complexity || 'Media').replace(/[^a-z0-9]/gi, '_');
      ...
      sections: [
        {
          ...
          children: [
              ...
              new Paragraph({ text: `Artículo Expositivo`, heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 100 } }),
              new Paragraph({ text: `Tema: ${formValue.topic}`, heading: HeadingLevel.HEADING_2, alignment: AlignmentType.CENTER, spacing: { after: 300 } }),
              ...
              new Paragraph({ text: `Vocabulario: ${formValue.vocabulary}`, alignment: AlignmentType.CENTER, style: "SubtleEmphasis" }),
  ...
  get sectionCtrl(): AbstractControl | null { return this.articleForm.get('section'); }
  get complexityCtrl(): AbstractControl | null { return this.articleForm.get('complexity'); }
  get vocabularyCtrl(): AbstractControl | null { return this.articleForm.get('vocabulary'); }
  get topicCtrl(): AbstractControl | null { return this.articleForm.get('topic'); }
}
