...
import { PretifyPipe } from '../../shared/pipes/pretify.pipe'; // Correct path
...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="proverb-generator-card"> <mat-card-header>
    <mat-card-title>Generador de Refranes</mat-card-title>
    <mat-card-subtitle>Encuentra refranes y su significado para tus clases</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="proverbForm" (ngSubmit)="onSubmit()" class="proverb-form">
        <div class="form-row">
          ...
          <mat-form-field appearance="outline" class="form-field">
            <mat-label>Complejidad / Familiaridad</mat-label>
            <mat-select formControlName="complexity" required>
               @for (level of complexityLevels; track level) {
                <mat-option [value]="level">{{ level }}</mat-option>
              }
            </mat-select>
             @if (complexityCtrl?.invalid && complexityCtrl?.touched) {
              <mat-error>Selecciona la complejidad.</mat-error>
            }
          </mat-form-field>
        </div>
        ...
        <div class="form-row">
          <mat-form-field appearance="outline" class="form-field">
            <mat-label>Nivel de Vocabulario (en explicación)</mat-label>
            <mat-select formControlName="vocabulary" required>
               @for (level of vocabularyLevels; track level) {
                <mat-option [value]="level">{{ level }}</mat-option>
              }
            </mat-select>
             @if (vocabularyCtrl?.invalid && vocabularyCtrl?.touched) {
              <mat-error>Selecciona el nivel de vocabulario.</mat-error>
            }
          </mat-form-field>

           <mat-form-field appearance="outline" class="form-field">
            <mat-label>Tema del refrán (Opcional)</mat-label>
            <input matInput formControlName="topic" placeholder="Ej: Trabajo, Paciencia, Amistad">
          </mat-form-field>
        </div>

        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="proverbForm.invalid || isGenerating()">
            @if (isGenerating()) {
              ...
              <ng-container>
                 <mat-icon>format_quote</mat-icon> Generar Refranes
              </ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="proverb-result">
        ...
        <div class="proverb-result-content" [innerHTML]="generatedProverbs().replaceAll('\n', '<br>')"></div>

        <div class="result-actions">
          <button mat-stroked-button color="primary" (click)="goBack()">
            <mat-icon>arrow_back</mat-icon> Volver
          </button>
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedProverbs() || generatedProverbs().startsWith('Ocurrió un error')">
  ...
  styles: [`
    :host { display: block; }
    .proverb-generator-card { margin: 0 auto; padding: 15px 25px 25px 25px; }
    .proverb-form { margin-top: 16px; display: flex; flex-direction: column; gap: 15px; }
    ...
    .proverb-result { margin-top: 20px; }
    .proverb-result h3 { margin-bottom: 15px; }
    ...
    /* Style the meaning if possible (depends on AI format) */
     .proverb-result-content span[style*="italic"] { /* Example: Italic text */
        display: block;
        margin-top: 0.5em;
        font-style: italic;
        color: #555;
...
export class ProverbGeneratorComponent implements OnInit, OnDestroy {
  ...
  readonly complexityLevels = ['Común / Sencillo', 'Menos Común / Figurado', 'Complejo / Antiguo']; // Renamed from length
  ...
  #loadSections(): void {
   ...
   #getComplexityInstruction(complexitySelection: string): string {
       switch(complexitySelection) {
           case 'Común / Sencillo': return 'muy comunes y de significado bastante literal o fácil de deducir';
           case 'Menos Común / Figurado': return 'algo menos comunes o que usen lenguaje más figurado';
           case 'Complejo / Antiguo': return 'más complejos, quizás más antiguos o con un significado menos obvio';
           default: return 'de complejidad y familiaridad media';
       }
   }
   ...
   #getVocabularyInstruction(vocabularySelection: string): string {
       ...
       switch(vocabularySelection) {
           case 'Reducido': return 'un vocabulario muy sencillo y directo en la explicación';
           case 'Medio': return 'un vocabulario estándar en la explicación, apropiado para la edad';
           case 'Amplio': return 'un vocabulario un poco más rico en la explicación';
           default: return 'un vocabulario estándar en la explicación';
       }
   }
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.generatedProverbs.set(result?.response || 'No se pudieron generar los refranes.');
      this.showResult.set(true);
    } catch (error) {
       this.generatedProverbs.set('Ocurrió un error al generar los refranes. Por favor, inténtalo de nuevo.');
  ...
  downloadDocx(): void {
    ...
    if (!proverbsText || proverbsText.startsWith('Ocurrió un error')) return;
    ...
    const complexityName = (formValue.complexity || 'Complejidad').substring(0,10).replace(/[^a-z0-9]/gi, '_');
    const topicName = (formValue.topic || 'Refranes').substring(0,15).replace(/[^a-z0-9]/gi, '_');
    ...
    proverbBlocks.forEach((block, index) => {
        ...
        const proverbPart = (index === 0 && !block.trim().startsWith("Refrán:") ? block.split(/\n\s*Significado:/)[0] : parts[0]).replace(/^Refrán:/, '').trim();
        const meaningPart = parts.length > 1 ? parts[1].trim() : null;

        if (proverbPart) {
            paragraphs.push(new Paragraph({
                children: [new TextRun({ text: "Refrán:", bold: true, break: 1 }), new TextRun({ text: ` "${proverbPart}"`, italics: true, break: 1 })], // Quote the proverb
                spacing: { after: 100, before: index > 0 ? 240 : 0 }, // Add space before new proverb
            }));
        }
        if (meaningPart) {
             paragraphs.push(new Paragraph({
                children: [new TextRun({ text: "Significado:", bold: true }), new TextRun({ text: ` ${meaningPart}` })],
      ...
      sections: [
        {
          properties: {},
          children: [
              new Paragraph({ text: `Refranes Generados`, heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 300 } }),
              new Paragraph({ text: `Curso: ${this.getSectionDisplay(section!)}`, alignment: AlignmentType.CENTER, style: "SubtleEmphasis"}),
              new Paragraph({ text: `Complejidad: ${formValue.complexity}`, alignment: AlignmentType.CENTER, style: "SubtleEmphasis" }),
              new Paragraph({ text: `Vocabulario (Explicación): ${formValue.vocabulary}`, alignment: AlignmentType.CENTER, style: "SubtleEmphasis" }),
  ...
  get sectionCtrl(): AbstractControl | null { return this.proverbForm.get('section'); }
  get complexityCtrl(): AbstractControl | null { return this.proverbForm.get('complexity'); }
  get vocabularyCtrl(): AbstractControl | null { return this.proverbForm.get('vocabulary'); }
  get topicCtrl(): AbstractControl | null { return this.proverbForm.get('topic'); }
}
