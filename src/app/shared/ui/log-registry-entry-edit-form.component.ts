...
import { createLogRegistryEntrySuccess, loadSections, loadStudentsBySection, selectAllClassSections, updateLogRegistryEntry, updateLogRegistryEntrySuccess } from '../../store'
...
export class LogRegistryEntryEditFormComponent implements OnInit {
	...
	update() {
		...
		this.#store.dispatch(updateLogRegistryEntry({ id: this.id, data: entry }))
		this.#actions$.pipe(ofType(updateLogRegistryEntrySuccess), takeUntil(this.#destroy$)).subscribe(() => {
			this.dialogRef.close()
		})
	}
