...
export class ChecklistGeneratorComponent {
        ...
        this.classSectionService.findSections().subscribe({
            next: sections => {
                if (sections.length) {
                    ...
                    this.checklistForm.get('section')?.setValue(sections[0]._id);
    ...
    onTitleEdit(event: any) {
        if (this.checklist)
            this.checklist.title = event.target.value;
    }

    onActivityEdit(event: any) {
        if (this.checklist)
            this.checklist.activity = event.target.value;
    }
            ...
            this.competenceService.findAll(filters).subscribe({
                next: competence => {
                    if (competence.length) {
                        ...
                        this.compNames = competence.map(c => this.pretify(c.name)).join(', ')
                        if (this.checklist)
    ...
    save() {
        if (this.checklist) {
            ...
            this.checklistService.create(list).subscribe({
                next: checklist => {
                    if (checklist._id) {
                        this.router.navigateByUrl("/checklists/" + checklist._id).then(() => {
                            this.sb.open('Se ha guardado la lista de cotejo.', 'Ok', { duration: 2500 });
                        });
                    } else {
    ...
    generate() {
        ...
        const { year, level } = this.section;
        const subject = this.pretify(this.checklistForm.get('subject')?.value || '');
        const activity = this.pretify(this.checklistForm.get('activity')?.value || '');
        const activityType = this.pretify(this.checklistForm.get('activityType')?.value || '');
        ...
        this.aiService.geminiAi(query).subscribe({
            next: res => {
                ...
                if (arr)
                    ...
                    this.sb.open('Ha ocurrido un error al generar tu lista de cotejo.', 'Ok', {duration: 2500});
            },
            error: err => {
                this.generating = false;
                this.sb.open('Ha ocurrido un error al generar tu lista de cotejo.', 'Ok', {duration: 2500});
