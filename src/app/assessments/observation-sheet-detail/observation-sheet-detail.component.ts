...
export class ObservationSheetDetailComponent implements OnInit {
    ...
    this.guideService.find(this.id).subscribe({
      next: guide => {
        if (guide._id) {
          ...
          this.studentService.findBySection(guide.section._id).subscribe(students => {
            if (students.length) {
  ...
  download() {
    if (!this.observationGuide)
      return;
    const title = this.observationGuide.title;
    this.sb.open('Estamos preparando la descarga, espera un momento.', 'Ok', { duration: 2500 });
    if (this.observationGuide.individual) {
      this.students.forEach((student, i) => {
        this.pdfService.createAndDownloadFromHTML('guide-'+i, `Guia de observacion ${title} para ${student.firstname} ${student.lastname}`);
      });
    } else {
      this.pdfService.createAndDownloadFromHTML('guide', 'Guia de observacion: ' + title);
    }
  }
}
