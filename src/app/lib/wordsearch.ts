  ...
  for (let i = 0; i < words.length; i++) {
    ...
    while (attempts < MAXATTEMPTS) {
      ...
      for (let l = 0; l < word.length; l++) {
        ...
        if (opts.color) grid[y][x] = '\x33[' + (colorno + 41) + 'm' + grid[y][x] + '\x33[0m';
    ...
    for (let j = 0; j < grid[i].length; j++)
      if (!grid[i][j]) {
        ...
        grid[i][j] = (opts.letters || LETTERS).charAt(
            Math.floor(Math.random() * (opts.letters || LETTERS).length)
        );
      }
 ...
 * and velocity
 */
function directioninfo(word: string, direction: number, width: number, height: number) {
