import { Component, signal, inject, ChangeDetectionStrategy, OnInit, OnDestroy, ViewEncapsulation } from '@angular/core';
import { FormBuilder, ReactiveFormsModule, Validators, AbstractControl } from '@angular/forms';
import { Subject, firstValueFrom, takeUntil, tap, catchError, EMPTY, finalize } from 'rxjs';
...
import { AiService } from '../services/ai.service'; // Assuming standard AI service path
import { ClassSectionService } from '../services/class-section.service'; // Correct path

// --- Interfaces ---
import { ClassSection } from '../interfaces/class-section'; // Correct path
...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="tongue-twister-card"> <mat-card-header>
    <mat-card-title>Generador de Trabalenguas</mat-card-title>
    <mat-card-subtitle>Crea trabalenguas divertidos para tus clases</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="tongueTwisterForm" (ngSubmit)="onSubmit()" class="tongue-twister-form">
        <div class="form-row">
          <mat-form-field appearance="outline" class="form-field">
            ...
            <mat-select formControlName="section" required>
              @if(isLoadingSections()){
                <mat-option disabled>
                  <mat-spinner diameter="20" style="display: inline-block; margin-right: 8px;"></mat-spinner> Cargando...
                </mat-option>
              } @else {
        ...
        <div class="form-row">
          <mat-form-field appearance="outline" class="form-field full-width-field"> <mat-label>Tema para el trabalenguas (Opcional)</mat-label>
            <input matInput formControlName="topic" placeholder="Ej: animales, frutas, números">
          </mat-form-field>
        </div>

        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="tongueTwisterForm.invalid || isGenerating()">
            @if (isGenerating()) {
              <mat-spinner diameter="20" color="accent" style="display: inline-block; margin-right: 8px;"></mat-spinner>
              Generando...
            } @else {
				<ng-container>
              <mat-icon>record_voice_over</mat-icon> Generar Trabalenguas
			</ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="tongue-twister-result">
        ...
        <div class="tongue-twister-result-page" [innerHTML]="generatedTongueTwister().replaceAll('\n', '<br>')">
          </div>
        ...
        <div class="result-actions">
          ...
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedTongueTwister() || generatedTongueTwister().startsWith('Ocurrió un error')">
...
export class TongueTwisterGeneratorComponent implements OnInit, OnDestroy {
  ...
  #loadSections(): void {
    ...
    this.#sectionService.findSections().pipe( // Assuming getSections exists
      takeUntil(this.#destroy$),
      tap(sections => this.sections.set(sections || [])),
      catchError(error => {
        console.error('Error loading sections:', error);
        this.#snackBar.open('Error al cargar las secciones.', 'Cerrar', { duration: 5000 });
        return EMPTY;
      }),
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.generatedTongueTwister.set(result?.response || 'No se pudo generar el trabalenguas.');
      this.showResult.set(true);
    } catch (error) {
      console.error('Error generating tongue twister:', error);
      this.generatedTongueTwister.set('Ocurrió un error al generar el trabalenguas. Por favor, inténtalo de nuevo.');
      this.showResult.set(true);
      this.#snackBar.open('Error al contactar el servicio de IA', 'Cerrar', { duration: 5000 });
    } finally {
  ...
  downloadDocx(): void {
    ...
    if (!tongueTwisterText || tongueTwisterText.startsWith('Ocurrió un error')) return;
    ...
    const topicName = (formValue.topic || 'General').substring(0,15).replace(/[^a-z0-9]/gi, '_');
      ...
      sections: [
        {
          properties: {},
          children: [
              new Paragraph({ children: [new TextRun({ text: `Trabalenguas Generado`, bold: true, size: 28 })], spacing: { after: 300 } }),
              new Paragraph({ children: [new TextRun({ text: `Sección: ${this.getSectionDisplay(section!)}`, size: 24 })], spacing: { after: 100 } }),
              new Paragraph({ children: [new TextRun({ text: `Asignatura: ${formValue.subject}`, size: 24 })], spacing: { after: 100 } }),
              ...(formValue.topic ? [new Paragraph({ children: [new TextRun({ text: `Tema: ${formValue.topic}`, size: 24 })], spacing: { after: 400 } })] : [new Paragraph({ text: '', spacing: { after: 400 }})]), // Add topic if present or just spacing
