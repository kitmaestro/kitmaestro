...
import { ImageRun, Table, WidthType, TableRow, TableCell, Paragraph, TextRun, AlignmentType, PageOrientation, HeadingLevel, Packer, Document } from 'docx';
    ...
    const contentsTable = new Table({
      ...
      rows: [
        ...
        new TableRow({
          children: [
            new TableCell({
              children: plan.contents.flatMap(block => block.concepts.map(concept => new Paragraph(concept)))
            }),
            new TableCell({
              children: plan.contents.flatMap(block => block.procedures.map(procedure => new Paragraph(procedure)))
            }),
            new TableCell({
              children: plan.contents.flatMap(block => block.attitudes.map(attitude => new Paragraph(attitude)))
            })
          ]
        })
      ]
    ...
    const activitiesTable = new Table({
      ...
      rows: [
        ...
        new TableRow({
          children: [
            new TableCell({
              children: plan.teacherActivities.flatMap(block => block.activities.map(concept => new Paragraph(concept)))
            }),
            new TableCell({
              children: plan.contents.flatMap(block => block.procedures.map(procedure => new Paragraph(procedure)))
            }),
      ...
      sections: [
          ...
          children: [
            ...
            new Paragraph({
              children: [
                new TextRun({
                  // color: '#000000',
                  text: this.pretify(plan.section.year) + " de " + this.pretify(plan.section.level),
                })
              ],
              heading: HeadingLevel.HEADING_2,
              alignment: AlignmentType.CENTER,
            ...
            new Paragraph({
              children: [
                new TextRun({
                  // color: '#000000',
                  text: "Asignatura" + (plan.subjects.length > 1 ? "s:" : ":"),
                })
              ],
              heading: HeadingLevel.HEADING_2,
              alignment: AlignmentType.CENTER,
        ...
        // body
        {
          ...
          children: [
            ...
            ...plan.competence.map(c => new Paragraph({
              text: this.pretifyCompetence(c.name, plan.section.level || 'PRIMARIA'),
              bullet: {
                level: 0
              }
            })),
            new Paragraph({ text: "Competencias Específicas del Grado", heading: HeadingLevel.HEADING_3 }),
            ...plan.competence.flatMap(c => c.entries.map(entry => new Paragraph({
              text: entry,
              bullet: {
                level: 0
              }
            ...
            ...plan.competence.flatMap(c => c.criteria.map(entry => new Paragraph({
              text: entry,
              bullet: {
                level: 0
              }
            }))),
            new Paragraph({ text: (plan.subjects.length > 1 ? "Ejes Transversales: " : "Eje Transversal: ") + plan.mainThemeCategory, heading: HeadingLevel.HEADING_3 }),
            ...plan.mainThemes.flatMap(theme => theme.topics.map(entry => new Paragraph({
              text: entry,
              bullet: {
                level: 0
              }
            }))),
            new Paragraph({ text: (plan.subjects.length > 1 ? "Asignaturas" : "Asignatura"), heading: HeadingLevel.HEADING_3 }),
            ...plan.subjects.flatMap(subject => new Paragraph({
            ...
            ...plan.strategies.flatMap(strategy => new Paragraph({
              text: strategy,
              bullet: {
                level: 0
              }
            ...
            ...plan.contents.flatMap(content => content.achievement_indicators.map(indicator => new Paragraph({
              text: indicator,
              bullet: {
                level: 0
              }
            ...
            ...plan.instruments.map(instrument => new Paragraph({ text: instrument, bullet: { level: 0 } })),
            new Paragraph({
              text: "Medios y Recursos",
              heading: HeadingLevel.HEADING_3
            }),
            ...plan.resources.map(resource => new Paragraph({ text: resource, bullet: { level: 0 } })),
          ]
        }
      ]
    });
