...
export class ClassScheduleService {
  ...
  create(schedule: ClassSchedule): Observable<ClassSchedule> {
    return this.http.post<ClassSchedule>(this.apiBaseUrl, schedule, this.config);
  }

  update(id: string, schedule: any): Observable<ApiUpdateResponse> {
    return this.http.patch<ApiUpdateResponse>(this.apiBaseUrl + id, schedule, this.config);
  }
