...
export class TestService {
  ...
  async download(test: Test) {
		...
		sections: [
			{
				...
				children: test.body.split('\n').map(text => {
					...
					if (text.startsWith('## '))
						return this.heading(text.slice(3), HeadingLevel.HEADING_2);
					
					if (text.startsWith('### '))
						return this.heading(text.slice(4), HeadingLevel.HEADING_3);
					...
					if (text.startsWith('|-'))
						return this.table(text.slice(1).replaceAll('-', ''));
	...
	saveAs(blob, `Examen de ${this.pretify(test.subject)} de ${this.pretify(test.section.year)} de ${this.pretify(test.section.level)}.docx`);
  }
  ...
  private heading(text: string, level: any = HeadingLevel.HEADING_1): Paragraph {
  ...
  private table(text: string): Table {
	return new Table({
		rows: text.split('|').map(s => {
			return new TableRow({
				children: [
					new TableCell({ children: [this.p(s)] })
				]
			})
		})
	});
