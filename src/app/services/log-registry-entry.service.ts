...
export class LogRegistryEntryService {
  ...
  private apiBaseUrl = isDevMode() ? 'http://localhost:3000/log-registry-entries/' : 'http://api.kitmaestro.com/log-registry-entries/';
  private config = {
  ...
  find(id: string): Observable<LogRegistryEntry> {
    return this.http.get<LogRegistryEntry>(this.apiBaseUrl + id, this.config);
  }

  create(plan: LogRegistryEntry): Observable<LogRegistryEntry> {
    return this.http.post<LogRegistryEntry>(this.apiBaseUrl, plan, this.config);
  }
