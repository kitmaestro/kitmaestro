...
Importante: NO HAGAS PREGUNTAS DE SEGUIMIENTO NI SUGIERAS QUE PUEDO PREGUNTAR YA QUE ESTO NO ME ES POSIBLE Y RESPONDE EN FORMATO MARKDOWN SIN TABLAS NI EMOTICONES.`;
        this.aiService.geminiAi(query).subscribe({
            ...
            error: err => {
                ...
                this.loading = false;
                this.sb.open('Ha ocurrido un error al generar la adaptacion. Intentalo de nuevo.', 'Ok', { duration: 2500 });
            }
        });
    }
    ...
    async download() {
        ...
        this.loading = true;
        const text: Test = { body: this.generated, section, subject: data.subject, user: this.user } as any;
