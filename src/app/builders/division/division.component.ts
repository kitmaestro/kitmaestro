...
export class DivisionComponent implements OnInit {
	...
	generateDivision(
		maxDividendDigits: number,
		maxDivisorDigits: number,
		exact = false,
	): Division {
		...
		if (exact) {
			...
			divisor = Math.floor(
				Math.pow(10, maxDivisorDigits - 1) +
					Math.random() *
						(Math.pow(10, maxDivisorDigits) -
							Math.pow(10, maxDivisorDigits - 1)),
			);
			...
			dividend =
				divisor *
				Math.floor(
					Math.pow(10, maxDividendDigits - maxDivisorDigits - 1) +
						Math.random() *
							(Math.pow(
								10,
								maxDividendDigits - maxDivisorDigits,
							) -
								Math.pow(
									10,
									maxDividendDigits - maxDivisorDigits - 1,
								)),
				);
			...
			dividend = Math.floor(
				Math.pow(10, maxDividendDigits - 1) +
					Math.random() *
						(Math.pow(10, maxDividendDigits) -
							Math.pow(10, maxDividendDigits - 1)),
			);
	...
	generateDivisions() {
		const { dividendDigits, divisorDigits, resultType, size } =
			this.divisionsForm.value;

		if (!dividendDigits || !divisorDigits || !resultType || !size) return;

		this.divisions = [];

		for (let i = 0; i < size; i++) {
			const exercise: Division = this.generateDivision(
				dividendDigits,
				divisorDigits,
				resultType === 'exact',
			);
