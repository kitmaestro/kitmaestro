...
import { Subject, firstValueFrom, takeUntil, tap, catchError, EMPTY, finalize, Observable } from 'rxjs';
...
@Component({
  ...
  // --- Inline Template ---
  template: `
<mat-card class="hook-generator-card"> <mat-card-header>
    <mat-card-title>Generador de Ganchos para Clases</mat-card-title>
    <mat-card-subtitle>Crea ideas atractivas para iniciar tus lecciones</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="hookForm" (ngSubmit)="onSubmit()" class="hook-form">
        <div class="form-row">
          ...
          <mat-form-field appearance="outline" class="form-field">
            ...
            <mat-select formControlName="subject" required>
               @for (subject of availableSubjects(); track subject) {
                <mat-option [value]="subject | pretify">{{ subject | pretify }}</mat-option>
              }
        ...
        <div class="form-row">
          <mat-form-field appearance="outline" class="form-field full-width-field">
            <mat-label>Tema de la clase (Opcional)</mat-label>
            <input matInput formControlName="topic" placeholder="Ej: El ciclo del agua, La Revolución Francesa">
          </mat-form-field>
        </div>

        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="hookForm.invalid || isGenerating()">
            @if (isGenerating()) {
              <mat-spinner diameter="20" color="accent" class="inline-spinner"></mat-spinner>
              Generando...
            } @else {
				<ng-container>
              		<mat-icon>flare</mat-icon> Generar Ganchos
			</ng-container>
            }
          </button>
        </div>
    ...
    @if (showResult()) {
      <div class="hook-result">
        ...
        <div class="result-actions">
          ...
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedHooks() || generatedHooks().startsWith('Ocurrió un error')">
  ...
  styles: [`
    ...
    .hook-generator-card { /* No max-width */
      margin: 0 auto;
    ...
    .hook-result-content ul, .hook-result-content ol {
        padding-left: 25px;
        margin-top: 10px;
        margin-bottom: 10px;
    }
...
export class ClassHookGeneratorComponent implements OnInit, OnDestroy {
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.generatedHooks.set(result?.response || 'No se pudieron generar los ganchos.');
      this.showResult.set(true);
    } catch (error) {
       this.generatedHooks.set('Ocurrió un error al generar los ganchos. Por favor, inténtalo de nuevo.');
    ...
    // Create paragraphs, splitting by newline characters
     const paragraphs = hooksText.split('\n').map(
      (line) => {
          ...
          if (trimmedLine.match(/^(\d+\.|-|\*)\s+/)) { // Looks like a list item
             return new Paragraph({
                 children: [new TextRun(trimmedLine)],
                 bullet: { level: 0 }, // Add bullet point
                 spacing: { after: 150 }
             });
          } else if (trimmedLine.length > 0 && trimmedLine.length < 80 && !trimmedLine.includes(':')) { // Potentially a sub-heading/hook title
             return new Paragraph({
                 children: [new TextRun({ text: trimmedLine, bold: true })],
                 spacing: { after: 180 }
             });
          }
          return new Paragraph({ // Regular text
      ...
      sections: [
        {
          properties: {},
          children: [
              new Paragraph({ children: [new TextRun({ text: `Ganchos para la Clase`, bold: true, size: 28, color: "336699" })], alignment: AlignmentType.CENTER, spacing: { after: 300 } }), // Title
              ...
              ...(formValue.topic ? [new Paragraph({ children: [new TextRun({ text: `Tema: ${formValue.topic}`, size: 24 })], spacing: { after: 400 } })] : [new Paragraph({ text: '', spacing: { after: 400 }})]),
  ...
  get sectionCtrl(): AbstractControl | null { return this.hookForm.get('section'); }
  get subjectCtrl(): AbstractControl | null { return this.hookForm.get('subject'); }
}
