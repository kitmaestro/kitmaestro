...
import { FormBuilder, ReactiveFormsModule, Validators, AbstractControl } from '@angular/forms'; // Use Reactive Forms
import { Subject, firstValueFrom, takeUntil, tap, catchError, EMPTY, finalize } from 'rxjs'; // RxJS imports
...
<mat-card class="icebreaker-card">
  <mat-card-header>
    ...
    <mat-card-subtitle>Crea actividades dinámicas para iniciar tus clases</mat-card-subtitle>
  </mat-card-header>

  <mat-card-content>
    @if (!showResult()) {
      <form [formGroup]="icebreakerForm" (ngSubmit)="onSubmit()" class="icebreaker-form">
        <div class="form-row">
        ...
        <div class="form-row">
           <mat-form-field appearance="outline" class="form-field">
            ...
            <mat-select formControlName="activityType" required>
             ...
             @if (activityTypeCtrl?.invalid && activityTypeCtrl?.touched) {
              <mat-error>Selecciona un tipo de actividad.</mat-error>
            }
          </mat-form-field>
        ...
        <div class="form-actions">
           <button mat-raised-button color="primary" type="submit" [disabled]="icebreakerForm.invalid || isGenerating()">
            @if (isGenerating()) {
              <mat-spinner diameter="20" color="accent" style="display: inline-block; margin-right: 8px;"></mat-spinner>
              Generando...
            } @else {
				<ng-container>
					<mat-icon>auto_awesome</mat-icon> Generar Rompehielos
				</ng-container>
            }
          </button>
        </div>
      ...
      <div class="icebreaker-result">
        ...
        <div class="icebreaker-result-page">
			@if (generatedIcebreaker()) {
				<markdown [data]="generatedIcebreaker()"></markdown>
			}
          </div>
        ...
        <div class="result-actions">
          ...
          <button mat-raised-button color="primary" (click)="downloadDocx()" [disabled]="!generatedIcebreaker() || generatedIcebreaker().startsWith('Ocurrió un error')">
...
export class IcebreakerGeneratorComponent implements OnInit, OnDestroy {
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.generatedIcebreaker.set(result?.response || 'No se pudo generar la actividad.');
      this.showResult.set(true); // Show the result view
    } catch (error) {
      console.error('Error generating icebreaker:', error);
      this.generatedIcebreaker.set('Ocurrió un error al generar la actividad. Por favor, inténtalo de nuevo.');
  ...
  downloadDocx(): void {
    ...
    if (!icebreakerText || icebreakerText.startsWith('Ocurrió un error')) return;
    ...
    const activityType = (formValue.activityType || 'Tipo').substring(0,15).replace(/[^a-z0-9]/gi, '_');
      ...
      sections: [
        {
          ...
          children: [
              new Paragraph({ children: [new TextRun({ text: `Rompehielos Generado`, bold: true, size: 28 })], spacing: { after: 300 } }), // Title
              ...
              new Paragraph({ children: [new TextRun({ text: `Tipo: ${formValue.activityType}`, size: 24 })], spacing: { after: 400 } }),
