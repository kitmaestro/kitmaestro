...
export class TodoFormComponent {
  ...
  constructor(
    ...
    if (this.data) {
      const {
        _id, title, description, completed, list
      } = this.data;
      this.todoEditForm.setValue({ _id, title, description, completed, list });
    }
  }
  ...
  update() {
    ...
    this.todoService.update(todo._id, todo).subscribe({
      ...
      error: (err) => {
        this.sb.open("Error al guardar: " + err.message, 'Ok', { duration: 2500 });
      }
    });
  }
}
