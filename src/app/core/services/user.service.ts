...
export class UserService {
	...
	getSettings(userId?: string): Observable<User> {
		return userId
			? this.#apiService.get<User>(this.#endpoint + userId)
			: this.#apiService.get<User>('auth/profile');
	}
	...
	update(
		...
		return this.#apiService.patch<{ success: boolean; data: User }>(
			this.#endpoint + id,
			user,
		);
	}
	...
	setPhotoUrl(photoURL: string): Observable<User> {
		return this.#apiService.patch<User>(this.#endpoint + 'auth/profile', {
			photoURL,
		});
	}
}
