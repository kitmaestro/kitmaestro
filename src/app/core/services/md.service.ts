...
export class MdService {
    ...
    private parseMd(mdString: string): Paragraph[] {
        ...
        for (const line of lines) {
            ...
            if (line.startsWith('# ')) {
                docxElements.push(
                    this.createStyledParagraph(line.substring(2), HeadingLevel.HEADING_1)
                );
                continue;
            }
            if (line.startsWith('## ')) {
                docxElements.push(
                    this.createStyledParagraph(line.substring(3), HeadingLevel.HEADING_2)
                );
                continue;
            }
            if (line.startsWith('### ')) {
                docxElements.push(
                    this.createStyledParagraph(line.substring(4), HeadingLevel.HEADING_3)
                );
                continue;
            }
