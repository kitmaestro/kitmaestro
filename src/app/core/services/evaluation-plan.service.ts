	...
	buildEvaluationPlanPrompt({
		...
		const evaluationContext = {
			...
			competencias: plan.competence.map((c) => ({
				competenciaFundamental: this.pretify(
					c.name,
					classSection.level,
				),
			...
			actividatesDeAprendizajeYEvaluacion: [
				...plan.teacherActivities,
				...plan.evaluationActivities,
			].map((a) => ({
...
Agrupa los bloques de por competencia y asignatura, no deben haber competencias repetidas para la misma asignatura. Asegúrate que la suma de las ponderaciones por area curricular sea igual a 100%.`;
	...
	async downloadAsDocx(plan: EvaluationPlan): Promise<boolean> {
		try {
			...
			saveAs(
				blob,
				`plan-evaluacion-${plan.classSection.name}-${plan.unitPlan.subjects.map((s) => this.pretify(s)).join('-')}.docx`,
			);
			return true;
		} catch (error) {
			console.error(error);
					...
					rows: [
								...
								new TableCell({
									children: [
										new Paragraph({
											children: [
												new TextRun({
													text: 'Evaluación según los participantes',
													bold: true,
												}),
											],
										}),
						...
						new TableRow({
							children: [
								...
								new TableCell({
									children: plan.evaluationTypes.map(
										(type) => new Paragraph(`- ${type}`),
									),
								}),
								new TableCell({
									children: plan.evaluationParticipants.map(
										(participant) =>
											new Paragraph(`• ${participant}`),
									),
								}),
							],
						}),
	...
	private createEvaluationAreaTable(area: EvaluationEntry): Table {
		const dataRows: TableRow[] = area.evaluationBlocks.flatMap(
			(block, i) => {
				const evaluationRow = new TableRow({
					children: [
						new TableCell({
							children: [
								new Paragraph(block.achievementIndicatorAspect),
							],
							width: {
								size: 25,
								type: WidthType.PERCENTAGE,
				...
				children: [
					...
					new TableCell({
						children: [
							...
							...area.achievementIndicators.map(
								(indicator) => new Paragraph(`• ${indicator}`),
							),
						],
						width: {
							size: 33.33,
					...
					new TableCell({
						children: [
							...
							...area.criteria.map(
								(criterion) => new Paragraph(`• ${criterion}`),
							),
						],
						width: {
							size: 33.34,
			...
			area.evaluationBlocks.forEach((block, blockIndex) => {
				if (blockIndex === 0) {
					...
					content +=
						...
						'┐\n';
					content +=
						'│ Aspecto Indicador  │ Evidencias         │ Ponderación │ Instrumento(s)     │\n';
					content +=
						'├' +
						'─'.repeat(20) +
						'┼' +
