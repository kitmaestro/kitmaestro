...
import { initialClassSectionsState, ClassSectionStateStatus } from './class-sections.models'
...
export const classSectionsReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: ClassSectionStateStatus.IDLING, error }),
    ),
    ...
    on(ClassSectionsActions.updateSectionSuccess, (state, { section }) => {
        ...
        return {
            ...
            status: ClassSectionStateStatus.IDLING,
            selectedSection: section,
            classSections: state.classSections.map(cs => cs._id === updatedSection._id ? updatedSection : cs),
        }
    }),
    on(ClassSectionsActions.deleteSectionSuccess, (state, { id }) => ({
        ...
        classSections: state.classSections.filter(section => section._id !== id),
    })),
)
