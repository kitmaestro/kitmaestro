			...
			switchMap(({ id, data }) =>
				this.#userSubscriptionService.updateReferral(id, data).pipe(
					map((subscription) => {
						...
						return UserSubscriptionsActions.updateSubscriptionSuccess(
							{ subscription },
						);
					}),
					catchError((error) => {
						this.#sb.open(
							'Error al actualizar referido',
							'Ok',
							timing,
						);
						return of(
			...
			switchMap(({ id }) =>
				this.#userSubscriptionService.deleteReferral(id).pipe(
					map(() => {
						...
						return UserSubscriptionsActions.deleteSubscriptionSuccess(
							{ id },
						);
					}),
					catchError((error) => {
						this.#sb.open(
							'Error al eliminar referido',
							'Ok',
							timing,
						);
						return of(
