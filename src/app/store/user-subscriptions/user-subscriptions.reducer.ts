...
export const userSubscriptionsReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: UserSubscriptionStateStatus.IDLING, error }),
    ),

    // Handle success cases
    on(UserSubscriptionsActions.loadSubscriptionSuccess, (state, { subscription }) => ({
        ...state,
        status: UserSubscriptionStateStatus.IDLING,
        selectedSubscription: subscription,
    })),
    on(UserSubscriptionsActions.loadSubscriptionsSuccess, (state, { subscriptions }) => ({
        ...state,
        status: UserSubscriptionStateStatus.IDLING,
        subscriptions,
    })),
    on(UserSubscriptionsActions.loadCurrentSubscriptionSuccess, (state, { subscription }) => ({
        ...state,
        status: UserSubscriptionStateStatus.IDLING,
        currentSubscription: subscription,
    })),
    on(UserSubscriptionsActions.createSubscriptionSuccess, (state, { subscription }) => ({
        ...state,
        status: UserSubscriptionStateStatus.IDLING,
        subscriptions: [...state.subscriptions, subscription],
        currentSubscription: subscription, // Asumimos que la nueva sub es la actual
    })),
    on(UserSubscriptionsActions.updateSubscriptionSuccess, (state, { subscription }) => {
    ...
    on(UserSubscriptionsActions.deleteSubscriptionSuccess, (state, { id }) => ({
        ...
        status: UserSubscriptionStateStatus.IDLING,
        selectedSubscription:
            state.selectedSubscription?._id === id ? null : state.selectedSubscription,
        currentSubscription:
            state.currentSubscription?._id === id ? null : state.currentSubscription,
