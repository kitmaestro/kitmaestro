...
export const logRegistryEntriesReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: LogRegistryEntryStateStatus.IDLING, error }),
    ),

    // Handle success cases
    on(LogRegistryEntriesActions.loadLogRegistryEntrySuccess, (state, { entry }) => ({
        ...state,
        status: LogRegistryEntryStateStatus.IDLING,
        selectedEntry: entry,
    })),
    on(LogRegistryEntriesActions.loadLogRegistryEntriesSuccess, (state, { entries }) => ({
        ...state,
        status: LogRegistryEntryStateStatus.IDLING,
        entries,
    })),
    on(LogRegistryEntriesActions.createLogRegistryEntrySuccess, (state, { entry }) => ({
        ...state,
        status: LogRegistryEntryStateStatus.IDLING,
        entries: [entry, ...state.entries],
    })),
    on(LogRegistryEntriesActions.updateLogRegistryEntrySuccess, (state, { entry: updatedEntry }) => ({
        ...state,
        status: LogRegistryEntryStateStatus.IDLING,
        selectedEntry:
            state.selectedEntry?._id === updatedEntry._id ? updatedEntry : state.selectedEntry,
        entries: state.entries.map(e => (e._id === updatedEntry._id ? updatedEntry : e)),
    })),
    on(LogRegistryEntriesActions.deleteLogRegistryEntrySuccess, (state, { id }) => ({
        ...state,
        status: LogRegistryEntryStateStatus.IDLING,
        selectedEntry: state.selectedEntry?._id === id ? null : state.selectedEntry,
