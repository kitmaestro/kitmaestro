...
import {
	...
	MainThemeStateStatus,
} from './main-themes.models';
...
export const mainThemesReducer = createReducer(
	...
	on(
		MainThemesActions.updateThemeSuccess,
		(state, { theme: updatedTheme }) => ({
			...state,
			status: MainThemeStateStatus.IDLING,
			selectedTheme:
				state.selectedTheme?._id === updatedTheme._id
					? updatedTheme
					: state.selectedTheme,
			themes: state.themes.map((t) =>
				t._id === updatedTheme._id ? updatedTheme : t,
			),
		}),
	),
	on(MainThemesActions.deleteThemeSuccess, (state, { id }) => ({
		...
		status: MainThemeStateStatus.IDLING,
		selectedTheme:
			state.selectedTheme?._id === id ? null : state.selectedTheme,
