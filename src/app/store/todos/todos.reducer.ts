...
export const todosReducer = createReducer(
    ...
    on(TodosActions.loadTodo, state => ({ ...state, status: TodoStateStatus.LOADING_TODO })),
    on(TodosActions.loadTodos, state => ({ ...state, status: TodoStateStatus.LOADING_TODOS })),
    on(TodosActions.loadTodosByList, state => ({
        ...state,
        status: TodoStateStatus.LOADING_LIST_TODOS,
    })),
    on(TodosActions.createTodo, state => ({ ...state, status: TodoStateStatus.CREATING_TODO })),
    on(TodosActions.updateTodo, state => ({ ...state, status: TodoStateStatus.UPDATING_TODO })),
    on(TodosActions.deleteTodo, state => ({ ...state, status: TodoStateStatus.DELETING_TODO })),
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: TodoStateStatus.IDLING, error }),
    ),
    ...
    on(TodosActions.updateTodoSuccess, (state, { todo: updatedTodo }) => ({
        ...
        status: TodoStateStatus.IDLING,
        selectedTodo:
            state.selectedTodo?._id === updatedTodo._id ? updatedTodo : state.selectedTodo,
        todos: state.todos.map(t => (t._id === updatedTodo._id ? updatedTodo : t)),
        listTodos: state.listTodos.map(t => (t._id === updatedTodo._id ? updatedTodo : t)),
    })),
    on(TodosActions.deleteTodoSuccess, (state, { id }) => ({
        ...state,
        status: TodoStateStatus.IDLING,
        selectedTodo: state.selectedTodo?._id === id ? null : state.selectedTodo,
