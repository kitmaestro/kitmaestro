...
export const estimationScalesReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: EstimationScaleStateStatus.IDLING, error }),
    ),
    ...
    on(EstimationScalesActions.updateScaleSuccess, (state, { scale: updatedScale }) => ({
        ...state,
        status: EstimationScaleStateStatus.IDLING,
        selectedScale:
            state.selectedScale?._id === updatedScale._id ? updatedScale : state.selectedScale,
        scales: state.scales.map(s => (s._id === updatedScale._id ? updatedScale : s)),
    })),
    on(EstimationScalesActions.deleteScaleSuccess, (state, { id }) => ({
        ...state,
        status: EstimationScaleStateStatus.IDLING,
        selectedScale: state.selectedScale?._id === id ? null : state.selectedScale,
