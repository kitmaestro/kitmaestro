		...
		this.#actions$.pipe(
			...
			switchMap(({ id }) =>
				this.#observationGuideService.find(id).pipe(
					map((guide) =>
						ObservationGuidesActions.loadGuideSuccess({ guide }),
					),
					catchError((error) => {
		...
		this.#actions$.pipe(
			...
			switchMap(() =>
				this.#observationGuideService.findAll().pipe(
					map((guides) =>
						ObservationGuidesActions.loadGuidesSuccess({ guides }),
					),
					catchError((error) => {
						this.#sb.open(
			...
			switchMap(({ guide }) =>
				this.#observationGuideService.create(guide).pipe(
					map((newGuide) => {
						...
						return ObservationGuidesActions.createGuideSuccess({
							guide: newGuide,
						});
					}),
					catchError((error) => {
			...
			switchMap(({ id, data }) =>
				this.#observationGuideService.update(id, data).pipe(
					map((updatedGuide) => {
						...
						return ObservationGuidesActions.updateGuideSuccess({
							guide: updatedGuide,
						});
					}),
					catchError((error) => {
			...
			switchMap(({ id }) =>
				this.#observationGuideService.delete(id).pipe(
					map(() => {
						...
						return ObservationGuidesActions.deleteGuideSuccess({
							id,
						});
					}),
					catchError((error) => {
