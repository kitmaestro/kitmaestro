...
export const observationGuidesReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: ObservationGuideStateStatus.IDLING, error }),
    ),
    ...
    on(ObservationGuidesActions.updateGuideSuccess, (state, { guide: updatedGuide }) => ({
        ...state,
        status: ObservationGuideStateStatus.IDLING,
        selectedGuide:
            state.selectedGuide?._id === updatedGuide._id ? updatedGuide : state.selectedGuide,
        guides: state.guides.map(g => (g._id === updatedGuide._id ? updatedGuide : g)),
    })),
    on(ObservationGuidesActions.deleteGuideSuccess, (state, { id }) => ({
        ...state,
        status: ObservationGuideStateStatus.IDLING,
        selectedGuide: state.selectedGuide?._id === id ? null : state.selectedGuide,
