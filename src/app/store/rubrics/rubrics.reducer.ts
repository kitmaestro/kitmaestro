...
export const rubricsReducer = createReducer(
	...
	on(
		RubricsActions.updateRubricSuccess,
		(state, { rubric: updatedRubric }) => ({
			...state,
			status: RubricStateStatus.IDLING,
			selectedRubric:
			...
			rubrics: state.rubrics.map((r) =>
				r._id === updatedRubric._id ? updatedRubric : r,
			),
		}),
	),
	on(RubricsActions.deleteRubricSuccess, (state, { id }) => ({
		...
		status: RubricStateStatus.IDLING,
		selectedRubric:
			state.selectedRubric?._id === id ? null : state.selectedRubric,
