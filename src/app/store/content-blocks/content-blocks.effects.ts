		...
		this.#actions$.pipe(
			...
			switchMap(({ id }) =>
				this.#contentBlockService.find(id).pipe(
					map((block) =>
						ContentBlocksActions.loadBlockSuccess({ block }),
					),
					catchError((error) => {
						this.#sb.open(
							'Error al cargar el bloque de contenido',
							'Ok',
							timing,
						);
						return of(
		...
		this.#actions$.pipe(
			...
			switchMap(({ block }) =>
				this.#contentBlockService.create(block).pipe(
					map((newBlock) => {
						this.#sb.open(
							'El bloque de contenido ha sido creado',
							'Ok',
							timing,
						);
						return ContentBlocksActions.createBlockSuccess({
							block: newBlock,
						});
					}),
					catchError((error) => {
						this.#sb.open(
							'Error al crear el bloque de contenido',
							'Ok',
							timing,
						);
						return of(
		...
		this.#actions$.pipe(
			...
			switchMap(({ id, data }) =>
				this.#contentBlockService.update(id, data).pipe(
					map((updatedBlock) => {
						this.#sb.open(
							'El bloque de contenido fue actualizado',
							'Ok',
							timing,
						);
						return ContentBlocksActions.updateBlockSuccess({
							block: updatedBlock,
						});
					}),
					catchError((error) => {
						this.#sb.open(
							'Error al actualizar el bloque de contenido',
							'Ok',
							timing,
						);
						return of(
		...
		this.#actions$.pipe(
			...
			switchMap(({ id }) =>
				this.#contentBlockService.delete(id).pipe(
					map(() => {
						this.#sb.open(
							'El bloque de contenido ha sido eliminado',
							'Ok',
							timing,
						);
					...
					catchError((error) => {
						this.#sb.open(
							'Error al eliminar el bloque de contenido',
							'Ok',
							timing,
						);
						return of(
