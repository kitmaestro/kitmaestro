...
import {
	...
	ContentBlockStateStatus,
} from './content-blocks.models';
...
export const contentBlocksReducer = createReducer(
	...
	on(
		ContentBlocksActions.updateBlockSuccess,
		(state, { block: updatedBlock }) => ({
			...state,
			status: ContentBlockStateStatus.IDLING,
			selectedBlock:
				state.selectedBlock?._id === updatedBlock._id
					? updatedBlock
					: state.selectedBlock,
	...
	on(ContentBlocksActions.deleteBlockSuccess, (state, { id }) => ({
		...
		status: ContentBlockStateStatus.IDLING,
		selectedBlock:
			state.selectedBlock?._id === id ? null : state.selectedBlock,
