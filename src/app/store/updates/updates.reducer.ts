...
export const updatesReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: UpdateStateStatus.IDLING, error }),
    ),
    ...
    on(UpdatesActions.updateUpdateSuccess, (state, { update: updatedUpdate }) => ({
        ...state,
        status: UpdateStateStatus.IDLING,
        selectedUpdate:
        ...
        updates: state.updates.map(u => (u._id === updatedUpdate._id ? updatedUpdate : u)),
    })),
    on(UpdatesActions.deleteUpdateSuccess, (state, { id }) => ({
        ...state,
        status: UpdateStateStatus.IDLING,
        selectedUpdate: state.selectedUpdate?._id === id ? null : state.selectedUpdate,
