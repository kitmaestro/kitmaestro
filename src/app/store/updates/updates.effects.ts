		...
		this.#actions$.pipe(
			...
			switchMap(({ id }) =>
				this.#updateService.find(id).pipe(
					map((update) =>
						UpdatesActions.loadUpdateSuccess({ update }),
					),
					catchError((error) => {
						this.#sb.open(
			...
			switchMap(({ update }) =>
				this.#updateService.create(update as any).pipe(
					map((newUpdate) => {
						...
						return UpdatesActions.createUpdateSuccess({
							update: newUpdate,
						});
					}),
					catchError((error) => {
						this.#sb.open(
							'Error al crear la actualización',
							'Ok',
							timing,
						);
						return of(
			...
			switchMap(({ id, data }) =>
				this.#updateService.update(id, data as any).pipe(
					map((updatedUpdate) => {
						...
						return UpdatesActions.updateUpdateSuccess({
		...
		this.#actions$.pipe(
			...
			switchMap(({ id }) =>
				this.#updateService.delete(id).pipe(
					map(() => {
						this.#sb.open(
							'La actualización ha sido eliminada',
							'Ok',
							timing,
						);
