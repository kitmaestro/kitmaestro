		...
		this.#actions$.pipe(
			...
			switchMap(() =>
				this.#authService.profile().pipe(
					...
					catchError((error) =>
						of(
							AuthActions.loadAuthUserFailure({
								error: error.message,
							}),
						),
					),
		...
		this.#actions$.pipe(
			...
			switchMap(() =>
				this.#authService.logout().pipe(
					map(() => {
						this.#sb.open('Se ha cerrado la sesion', 'Ok', {
							duration: 2500,
						});
			...
			switchMap(({ data }) =>
				this.#authService.signup(data).pipe(
					...
					catchError((error) => {
						...
						return of(
							AuthActions.signUpFailure({ error: error.message }),
						);
					}),
				),
			),
		...
		this.#actions$.pipe(
			...
			switchMap(({ email }) =>
				this.#authService.recover(email).pipe(
					map((response) =>
						AuthActions.passwordRecoverySuccess({ response }),
					),
					catchError((error) =>
						of(
		...
		this.#actions$.pipe(
			...
			switchMap(({ payload }) =>
				this.#authService.resetPassword(payload).pipe(
					map((response) =>
						AuthActions.passwordResetSuccess({ response }),
					),
					catchError((error) =>
						of(
