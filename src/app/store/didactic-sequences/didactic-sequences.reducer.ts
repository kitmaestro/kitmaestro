...
export const didacticSequencesReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: DidacticSequenceStateStatus.IDLING, error }),
    ),
    ...
    on(DidacticSequencesActions.loadSequencesSuccess, (state, { sequences }) => ({
        ...state,
        status: DidacticSequenceStateStatus.IDLING,
        sequences,
    })),
    on(DidacticSequencesActions.createSequenceSuccess, (state, { sequence }) => ({
        ...state,
        status: DidacticSequenceStateStatus.IDLING,
        sequences: [sequence, ...state.sequences],
    })),
    on(DidacticSequencesActions.updateSequenceSuccess, (state, { sequence: updatedSequence }) => ({
        ...state,
        status: DidacticSequenceStateStatus.IDLING,
        selectedSequence:
    ...
    on(DidacticSequencesActions.deleteSequenceSuccess, (state, { id }) => ({
        ...state,
        status: DidacticSequenceStateStatus.IDLING,
        selectedSequence: state.selectedSequence?._id === id ? null : state.selectedSequence,
