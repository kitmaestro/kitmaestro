        ...
        this.#actions$.pipe(
            ...
            switchMap(({ id }) =>
                this.#didacticSequenceService.findOne(id).pipe(
                    map(sequence => DidacticSequencesActions.loadSequenceSuccess({ sequence })),
                    catchError(error => {
                        this.#sb.open('Error al cargar la secuencia', 'Ok', timing)
                        return of(
                            DidacticSequencesActions.loadSequenceFailed({ error: error.message }),
                        )
                    }),
                ),
            ),
        ...
        this.#actions$.pipe(
            ...
            switchMap(({ filters }) =>
                this.#didacticSequenceService.findAll(filters).pipe(
                    map(sequences => DidacticSequencesActions.loadSequencesSuccess({ sequences })),
                    catchError(error => {
                        this.#sb.open('Error al cargar las secuencias', 'Ok', timing)
                        return of(
                            DidacticSequencesActions.loadSequencesFailed({ error: error.message }),
                        )
                    }),
                ),
            ),
        ...
        this.#actions$.pipe(
            ...
            switchMap(({ sequence }) =>
                this.#didacticSequenceService.create(sequence).pipe(
                    map(newSequence => {
                        this.#sb.open('La secuencia ha sido creada', 'Ok', timing)
                        return DidacticSequencesActions.createSequenceSuccess({
                            sequence: newSequence,
                        })
                    }),
                    catchError(error => {
                        this.#sb.open('Error al crear la secuencia', 'Ok', timing)
                        return of(
        ...
        this.#actions$.pipe(
            ...
            switchMap(({ id, data }) =>
                this.#didacticSequenceService.update(id, data).pipe(
                    map(updatedSequence => {
                        this.#sb.open('La secuencia fue actualizada', 'Ok', timing)
                        return DidacticSequencesActions.updateSequenceSuccess({
                            sequence: updatedSequence,
                        })
                    }),
                    catchError(error => {
                        this.#sb.open('Error al actualizar la secuencia', 'Ok', timing)
                        return of(
        ...
        this.#actions$.pipe(
            ...
            switchMap(({ id }) =>
                this.#didacticSequenceService.delete(id).pipe(
                    map(() => {
                        this.#sb.open('La secuencia ha sido eliminada', 'Ok', timing)
                        return DidacticSequencesActions.deleteSequenceSuccess({ id })
                    }),
                    catchError(error => {
                        this.#sb.open('Error al eliminar la secuencia', 'Ok', timing)
                        return of(
