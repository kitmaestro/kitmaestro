...
export const testsReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: TestStateStatus.IDLING, error }),
    ),
    ...
    on(TestsActions.updateTestSuccess, (state, { test: updatedTest }) => ({
        ...
        status: TestStateStatus.IDLING,
        selectedTest:
            state.selectedTest?._id === updatedTest._id ? updatedTest : state.selectedTest,
        tests: state.tests.map(t => (t._id === updatedTest._id ? updatedTest : t)),
    })),
    on(TestsActions.deleteTestSuccess, (state, { id }) => ({
        ...state,
        status: TestStateStatus.IDLING,
        selectedTest: state.selectedTest?._id === id ? null : state.selectedTest,
