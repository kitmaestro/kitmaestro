...
import { initialScoreSystemsState, ScoreSystemStateStatus } from './score-systems.models'
...
export const scoreSystemsReducer = createReducer(
    ...
    on(
        ...
        (state, { error }) => ({ ...state, status: ScoreSystemStateStatus.IDLING, error }),
    ),
    ...
    on(ScoreSystemsActions.updateSystemSuccess, (state, { system: updatedSystem }) => ({
        ...state,
        status: ScoreSystemStateStatus.IDLING,
        selectedSystem:
        ...
        systems: state.systems.map(s => (s._id === updatedSystem._id ? updatedSystem : s)),
    })),
    on(ScoreSystemsActions.deleteSystemSuccess, (state, { id }) => ({
        ...state,
        status: ScoreSystemStateStatus.IDLING,
        selectedSystem: state.selectedSystem?._id === id ? null : state.selectedSystem,
