  ...
  ngOnInit() {
    if (this.updateId) {
      this.updateService.find(this.updateId).subscribe({
        next: update => {
          if (update) {
            ...
            this.form.get('date')?.setValue(new Date(update.date).toISOString().split('T')[0]);
            this.form.get('type')?.setValue(update.type);
            this.form.get('description')?.setValue(update.description);
      ...
      this.authService.profile().subscribe(user => {
        if (user) {
          this.form.get('author')?.setValue(`${user.firstname} ${user.lastname}`);
        }
      })
    }
  }
      ...
      this.updateService.update(this.updateId, data).subscribe(res => {
        if (res.modifiedCount > 0) {
          this.router.navigateByUrl("/updates").then(() => {
            this.sb.open('Se ha publicado la actualizacion!', 'Ok', { duration: 2500 });
          })
        }
      })
    } else {
