...
import { Attendance, AttendanceCalendar, AttendanceWeek } from '../interfaces/attendance';
...
export class AttendanceDashboardComponent implements OnInit {
        ...
        setTimeout(() => {
            this.students.forEach(student => {
                const attendance: any = {
                    ...
                    student,
                    data: this.calendar.weeks.flatMap(w => w.days).map(day => ({ attendance: 'PRESENTE', date: parseInt(day.date) })),
                }
    ...
    fetchAttendance(year: number, month: number) {
        this.attendanceService.findAll({ section: this.section.value, year, month }).subscribe({
            next: attendance => {
    ...
    onSave() {
        ...
        this.attendance.forEach(attendance => {
            if (this.remote) {
                const sus = this.attendanceService.update(attendance._id, attendance).subscribe({
                    next: res => {
                        sus.unsubscribe();
                        count += 1;
                        if (count === this.attendance.length) {
                ...
                const sus = this.attendanceService.create(attendance).subscribe({
                    next: res => {
                        sus.unsubscribe();
                        count += 1;
                        if (count === this.attendance.length) {
    ...
    makeCalendar() {
        this.monthName = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'][this.month.value - 1];
        switch (this.mode) {
            case 'Matutina':
            case 'Vespertina':
            case 'Nocturna':
            case 'Jornada Extendida': {
                ...
                const dateStr = (d: Date) => d.getDate().toString().padStart(2, '0');
                    ...
                    if (i === 0) {
                        ...
                        if (firstDay === 0) {
                            ...
                            for (let j = 5; j > 0; j--) {
                                ...
                                week.days.push({ date: '', dayOfTheWeek: dayOfTheWeek(d) });
                            }
                            continue;
                        }
                        if (firstDay === 6) {
                        ...
                        for (let j = firstDay; j < 6; j++) {
                            week.days.push({ date: dateStr(date), dayOfTheWeek: dayOfTheWeek(date) });
                            date.setDate(date.getDate() + 1);
                        }
                        date.setDate(date.getDate() + 1);
                    } else {
    ...
    setHolidays() {
        this.holidays = [
            ...
            [6],
            [25],
        ][this.month.value - 1].map((v: number) => v.toString().padStart(2, '0'));
    }
